...
const manifest = {
  ART_SLOTS: [
    { id: "tree-pine", nameKo: "소나무", category: "tree", seasons: ["spring", "summer"] },
    { id: "tree-pine-autumn", nameKo: "소나무(가을)", category: "tree", seasons: ["autumn"] },
    { id: "tree-pine-winter", nameKo: "소나무(겨울)", category: "tree", seasons: ["winter"] },
    { id: "fish-koi", nameKo: "비단잉어", category: "fish", seasons: ["summer"] },
  ],
...
describe("entity reference library", () => {
  it("writes a new image/sidecar pair once and protects existing images and manual sidecars", () => {
    ...
    const imagePath = path.join(workspaceRoot, "art-src/나무/소나무/레퍼런스/pine.png");
    ...
    expect(fs.readFileSync(imagePath, "utf8")).toBe("new reference");
    write(workspaceRoot, "art-src/나무/소나무/레퍼런스/manual.png.json", "manual source record");
  ...
  it("preserves a dangling entry occupying an image filename", () => {
    ...
    const dir = path.join(workspaceRoot, "art-src/나무/소나무/레퍼런스");
    fs.mkdirSync(dir, { recursive: true });
    fs.symlinkSync(target, path.join(dir, "pine.png"), "junction");
    expect(refs.writeReferencePair({ workspaceRoot, category: "tree", entity: "tree-pine", manifest, filename: "pine.png", image: Buffer.from("new"), card: {} }).saved).toBe(false);
  ...
  it("rolls back only its exact new image if another writer claims the sidecar", () => {
    ...
    expect(() => refs.writeReferencePair({ workspaceRoot, category: "tree", entity: "tree-pine", manifest, filename: "pine.png", image: Buffer.from("new"), card: {} })).toThrow(/claimed/);
  ...
  it("does not delete a changed image when sidecar creation fails", () => {
    ...
    expect(() => refs.writeReferencePair({ workspaceRoot, category: "tree", entity: "tree-pine", manifest, filename: "pine.png", image: Buffer.from("new"), card: {} })).toThrow(/failed/);
    expect(fs.readFileSync(imagePath, "utf8")).toBe("other writer's image");
  });

  it("requires one explicit canonical entity and does not treat query values as categories", () => {
    const workspaceRoot = workspace();
    ...
    expect(refs.parseReferenceFetchArgs(["tree", "--entity", "tree-pine", "--limit", "3"], options)).toMatchObject({
      limit: 3, targets: [{ category: "tree", dir: path.join(workspaceRoot, "art-src/나무/소나무/레퍼런스") }],
    });
    expect(refs.parseReferenceFetchArgs(["fish", "--entity", "fish-koi", "--query", "tree"], options)).toMatchObject({ query: "tree" });
  ...
  it("rejects unknown, cross-category, traversal, missing, and ambiguous entity destinations before collection", () => {
    ...
    for (const args of [
      ...
      ["tree"], ["--all"], ["--all", "--entity", "tree-pine"], ["tree", "fish", "--entity", "tree-pine"],
  ...
  it("distinguishes duplicate basenames across entity references, including GIFs", () => {
    const workspaceRoot = workspace();
    const files = ["art-src/나무/소나무/레퍼런스/same.png", "art-src/물고기/비단잉어/레퍼런스/same.gif"];
  ...
  it("does not inspect frozen runs, generated or rejected art, nested references, or unregistered entities", () => {
    const workspaceRoot = workspace();
    const ignored = [
      "art-src/나무/소나무/작업회차/시험회차/고정입력/레퍼런스/no-card.png",
      "art-src/나무/소나무/작업회차/시험회차/review.json",
      "art-src/나무/소나무/작업회차/시험회차/고정입력/레퍼런스/orphan.png.json",
      "art-src/나무/소나무/생성본/2/공통/no-card.png",
      "art-src/나무/소나무/반려본/no-card.png",
      "art-src/나무/소나무/레퍼런스/nested/no-card.png",
      "art-src/공통화풍참고/모여봐요 동물의 숲/no-card.png",
      "art-src/나무/미등록/레퍼런스/no-card.png",
      "art-src/나무/소나무/레퍼런스/review.json",
      "art-src/reference/tree/old-layout.png",
      "art-src/tree/tree-pine/레퍼런스/old-layout.png",
    ];
  ...
  it("--check reports missing sidecars and orphan cards without any write or deletion", () => {
    const workspaceRoot = workspace();
    write(workspaceRoot, "art-src/나무/소나무/레퍼런스/no-card.png");
    write(workspaceRoot, "art-src/나무/소나무/레퍼런스/orphan.png.json", "{}");
    write(workspaceRoot, "art-src/공통화풍참고/NOTICE.md", "existing notice");
  ...
  it("writes current references and removes only orphan image sidecars inside eligible directories", () => {
    const workspaceRoot = workspace();
    pair(workspaceRoot, "art-src/나무/소나무/레퍼런스/pine.png");
    write(workspaceRoot, "art-src/나무/소나무/레퍼런스/orphan.gif.json", "{}");
    const preserved = ["art-src/나무/소나무/레퍼런스/metadata.json", "art-src/나무/소나무/작업회차/시험회차/고정입력/레퍼런스/orphan.png.json"];
    ...
    expect(fs.existsSync(path.join(workspaceRoot, "art-src/나무/소나무/레퍼런스/orphan.gif.json"))).toBe(false);
  ...
  it("keeps the old notice and orphan evidence when current metadata is invalid", () => {
    const workspaceRoot = workspace();
    write(workspaceRoot, "art-src/공통화풍참고/NOTICE.md", "old notice");
    pair(workspaceRoot, "art-src/나무/소나무/레퍼런스/pine.png");
    write(workspaceRoot, "art-src/나무/소나무/레퍼런스/pine.png.json", "null");
    write(workspaceRoot, "art-src/나무/소나무/레퍼런스/orphan.png.json", "{}");
    const before = snapshot(workspaceRoot);
  ...
  it("refuses reference directory junctions before reading or mutating their targets", () => {
    ...
    fs.mkdirSync(path.join(workspaceRoot, "art-src/나무/소나무"), { recursive: true });
    fs.symlinkSync(target, path.join(workspaceRoot, "art-src/나무/소나무/레퍼런스"), "junction");
    expect(() => refs.parseReferenceFetchArgs(["tree", "--entity", "tree-pine"], { workspaceRoot, manifest })).toThrow(/symlink/);
