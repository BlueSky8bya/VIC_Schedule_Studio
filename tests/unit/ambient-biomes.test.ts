...
describe("world/biomes — 열두 화면 지도(PLAN-20260904-004 §3)", () => {
  it("3×3 + 남쪽 바다 세 줄 = 12, 초원이 (0,0), 미니맵 줄은 6", () => {
    expect(Object.keys(BIOMES).length).toBe(12);
    expect(biomeAt(0, 0)).toBe("meadow");
    expect(BIOME_ROWS.flat().length).toBe(12);
  });
  ...
  it("바다: 세 해안 어디서 내려가도 먼바다, 한 번 더 깊은 바다, 좌우는 막힘, 위로는 내려온 해안으로", () => {
    expect(neighbor("tidal", "down")).toBe("shallow");
    expect(neighbor("sandy", "down")).toBe("shallow");
    expect(neighbor("rocky", "down")).toBe("shallow");
    expect(neighbor("shallow", "down")).toBe("sea");
    expect(neighbor("sea", "up")).toBe("shallow");
    expect(neighbor("shallow", "left")).toBeNull();
    ...
    expect(neighbor("shallow", "up", 1)).toBe("rocky");
    expect(neighbor("shallow", "up", -1)).toBe("tidal");
  ...
  it("화면 이동량: 초원→숲 (+1,0), 암석해안→먼바다 (0,+1)(x 접힘), 먼바다→해안 복귀는 가로 0", () => {
    ...
    expect(screenDelta("rocky", "shallow", 1)).toEqual([0, 1]);
    expect(screenDelta("shallow", "rocky", 1)).toEqual([0, -1]);
    expect(screenDelta("meadow", "deep")).toEqual([0, 4]);
  });
});
