...
// 단계 확대(100/125/150/175/200%)로 바꾼다. 여기는 순수 로직만(테스트 대상) — DOM 연결은 studio-shell.
//
// 왜 단계형인가: 연속 배율은 "지금 몇 %인지"를 방송 화면에서 설명하기 어렵고, 트랙패드에선
// 한 제스처에 wheel 이벤트가 수십 개 와서 순식간에 끝까지 가버린다. 예측 가능한 단계만 두고
...
// 175·200%는 2026-09-06 소유자 요청("혹시 모르니 확장") — 방송에서 더 크게 띄울 여지.
// 칸·글자·띠·요일 머리글이 전부 `--cal-zoom`에 비례하므로 단계를 늘리는 데 CSS 변경은 없다.
export const CAL_ZOOM_STEPS = [1, 1.25, 1.5, 1.75, 2] as const;
