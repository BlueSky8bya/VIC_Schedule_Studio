-- 0111: 검색어 제안 관련어 — 용언 어미 목록 보강(왤케·걸린·드신·나온). 0110 재정의.
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0111_public_search_suggest_endings.sql
...
as $$
  ...
  scored as materialized (
    select c.term, c.kind, c.w, c.day, c.grp,
      ...
      exists (select 1 from public.search_doc_words d where d.term = public.search_norm(c.term)) as is_doc,
      case
  ...
  -- 굴절형 판정을 해시 조인으로 미리(행마다 서브쿼리는 O(n²) — 실측 37초)
  inflect as (
    select distinct s.tn from scored s join scored c2 on c2.tn = substr(s.tn, 1, length(s.tn) - 1)
    where length(s.tn) >= 3
  ),
  direct0 as (
    ...
    where s.m > 0 and length(s.tn) >= 2 and s.tn <> q.qn and s.tn not in (select w from stop)
      and s.tn !~ '(님의|님과|님이|님은|님을|님도|님께|님한테|께서|에서|으로|까지|부터|들의|들이|들은|들을|이서|에게|한테|처럼|보다|마다|조차|밖에)$'
      -- 일반 단어는 제목·챕터·일정에 나온 말만(채팅 전용어 '불독님께'·'빅바빅바'는 자동완성에 안 올림 — 연구 브리프 §4)
      and (s.kind <> 'term' or s.is_doc)
      -- 굴절형 제거는 일반 단어에만(이름은 '아마최'→'아마' 같은 오인이 난다)
      and not (s.kind = 'term' and s.tn in (select tn from inflect))
  ...
  related as (
    ...
    union all
    -- 같은 사람의 다른 이름: 실제로 쓰이는 것(제목·챕터·채팅 3방송↑ 또는 인물 그래프)만, 씨앗당 상위 3개 (소유자: 마이너 별명은 마이너스)
    select x.alt, (8.0 + ln(1 + x.docs))::numeric
    from (
      select s.alt, sd.tn, coalesce(t.docs, e.docs, 0) as docs,
             row_number() over (partition by sd.tn order by coalesce(t.docs, e.docs, 0) desc, s.alt) as rn
      from seeds sd
      join public.search_synonyms s on s.term = sd.tn and s.kind = 'syn' and s.source in ('seed-person', 'seed-web', 'seed-owner')
      left join public.search_terms t on t.term = s.alt
      left join public.search_entities e on e.name = s.alt
      where coalesce(t.docs, e.docs, 0) >= 3 and s.alt !~ '님$'
    ) x where x.rn <= 3
  ),
  ...
  rel_stop as (
    select unnest(array['역시','많이','사실','그럼','이건','일단','토리가','반응','소감','획득','가리','그거','저거','이거','이렇게','그렇게',
      ...
      '몰라','논란','맞네','화이팅','파이팅','대음','김치','여없노밥','토리도','토리는','토리가','토리야','아니야','그래요','네요','ㅋㅋ','ㅎㅎ',
      '가능','직접','사진','이름','친구','생일','시절','이벤트','방법','투표','가족','오늘','어제','내일','시작','마무리','준비','소개','확인','질문',
      '대답','설명','이야기','얘기','반응','댓글','채팅','시청','구독','후원','감사','인사','안녕','잠깐','휴식','종료','오프닝','엔딩']) as w
  ),
  common_cap as (
  ...
  related_ok as (
    ...
    where length(r.tn) >= 2 and r.tn <> q.qn
      ...
      and (r.tn !~ '(게|히|로|서|다|요|운|은|는|면|아|어|해|한|할|된|될|고|지|기|음|임|져|져서|했|됐|같|함|됨|줘|봐|자|네|죠|까|나요|세요|케|린|신|온|든|던|킨|친|린|왔|갔|왜|텐|텐데|는데|인데)$'
