...
-- 관련어(rel)도 같은 규칙. 멱등.
-- 적용: node scripts/apply-db.mjs db/migrations/0113_public_search_short_alt_boundary.sql
...
as $$
  ...
  q as (
    select
      ...
      -- 0113: 긴 대안어(qre_l)는 정규화 문자열에서, 두 글자 한글 대안어(qre_s)는 **원문**에서 찾는다.
      -- 초성 질의의 대안어는 ㄱ-ㅎ이라 자동으로 전부 qre_l로 간다(원문 대조는 애초에 불가).
      coalesce((select '(' || string_agg(a, '|') || ')' from unnest(q1.alts) a where a !~ '^[가-힣]{2}$'), '(?!)') as qre_l,
      coalesce((select '(' || string_agg(a, '|') || ')' from unnest(q1.alts) a where a ~ '^[가-힣]{2}$'), '(?!)') as qre_s,
      (select '(' || string_agg(a, '|') || ')' from unnest(q1.rels) a where a !~ '^[가-힣]{2}$') as qrel_l,
  ...
  evm as (
    select ev.*,
      ...
      ((case when q.is_cho then public.search_choseong(ev.tn) else ev.tn end) ~ q.qre_l or ev.traw ~ q.qre_s) as hit_t,
      ((case when q.is_cho then public.search_choseong(ev.tagn) else ev.tagn end) ~ q.qre_l or ev.graw ~ q.qre_s) as hit_g,
      ((case when q.is_cho then public.search_choseong(ev.dn) else ev.dn end) ~ q.qre_l or ev.draw ~ q.qre_s) as hit_d,
      ((q.qrel_l is not null and (ev.tn || ev.tagn || ev.dn) ~ q.qrel_l)
        or (q.qrel_s is not null and (ev.traw || ' ' || ev.graw || ' ' || ev.draw) ~ q.qrel_s)) as hit_r,
  ...
  vodm as (
    select v.*,
      ...
      ((case when q.is_cho then public.search_choseong(v.tn) else v.tn end) ~ q.qre_l or lower(v.title) ~ q.qre_s) as hit_t,
      ((q.qrel_l is not null and v.tn ~ q.qrel_l) or (q.qrel_s is not null and lower(v.title) ~ q.qrel_s)) as hit_r,
      (q.game_intent and (
  ...
  chm as (
    select c.title_no, c.sec, c.label, c.section, c.parent, c.song_kind,
      ...
      ((case when q.is_cho then c.label_cho else c.label_norm end) ~ q.qre_l or lower(c.label) ~ q.qre_s) as hit_t,
      (not q.is_cho and ((c.section_norm || '|' || c.parent_norm) ~ q.qre_l or lower(c.section || '|' || c.parent) ~ q.qre_s)) as hit_s,
      ((q.qrel_l is not null and (c.label_norm || '|' || c.section_norm || '|' || c.parent_norm) ~ q.qrel_l)
        or (q.qrel_s is not null and lower(c.label || '|' || c.section || '|' || c.parent) ~ q.qrel_s)) as hit_r,
...
comment on function public.search_public(uuid, text, integer) is
  '시청자 검색(0076~0113) — 오타 교정 + 공연 종류·종겜 의도·챕터 코너·동의어/관련어/줄임말/IDF/초성, 두 글자 낱말은 원문 인접일 때만. 공개 데이터만.';
