-- 0110: 검색어 제안 관련어 — 용언 어미(운·는·면·해·한…)·흔한 일반명사(가능·직접·사진·이름·친구…) 제외, '님께' 꼬리 제거. 0109 재정의.
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0110_public_search_suggest_stops.sql
...
as $$
  ...
  related_ok as (
    ...
    where length(r.tn) >= 2 and r.tn <> q.qn
      ...
      and (r.tn !~ '(게|히|로|서|다|요|운|은|는|면|아|어|해|한|할|된|될|고|지|기|음|임|져|져서|했|됐|같|함|됨|줘|봐|자|네|죠|까|나요|세요)$'
