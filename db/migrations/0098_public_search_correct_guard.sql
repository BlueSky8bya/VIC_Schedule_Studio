-- 0098: 오타 교정 억제 — 질의가 말뭉치(단어·게임·인물·제목·챕터·일정)에 그대로/앞부분으로 있으면 교정하지 않는다.
-- 실측(2026-09-18): 채팅 백필로 '뮤비'가 사전에 들어오자 "마비"가 '뮤비'로 교정돼 마비노기(46건)가 4건이 됐다.
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0098_public_search_correct_guard.sql
...
comment on function public.search_public(uuid, text, integer) is
  '시청자 검색(0076~0098) — 오타 교정(자모 편집거리) + 공연 종류·종겜 의도·챕터 코너·동의어/관련어/줄임말/IDF/초성. 공개 데이터만.';
