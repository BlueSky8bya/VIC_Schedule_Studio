-- 0095: 검색 — 일정 행에 그 날 다시보기 썸네일 동봉(지난 일정만; 미래 일정은 없음).
-- 소유자(2026-09-18): "일정 결과도 해당 다시보기 썸네일을 보여줬으면, 미래 일정이라 없으면 안 해도 됨".
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0095_public_search_event_thumb.sql
...
comment on function public.search_public(uuid, text, integer) is
  '시청자 검색(0076~0095) — 오타 교정(자모 편집거리) + 공연 종류·종겜 의도·챕터 코너·동의어/관련어/줄임말/IDF/초성. 공개 데이터만.';
