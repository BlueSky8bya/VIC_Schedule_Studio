-- 0117: 채팅 반응 행(chat_scored)이 챕터 CTE를 행마다 다시 훑던 것을 한 번으로.
--
-- 실측(EXPLAIN ANALYZE, '토리'): 전체 951ms 중 573ms가 chat_scored의 안티조인 두 개였다.
--   not exists (select 1 from chm  c where c.title_no = t.title_no and (c.hit_t or c.hit_s))
--   not exists (select 1 from vodm x where x.title_no = t.title_no and x.hit_t)
-- chm은 챕터 17,857행마다 적중 불리언 열두 개를 계산하는 CTE다. 채팅 후보마다 그걸 다시 훑으니
-- 중첩 반복이 된다. 제외할 title_no 목록을 **한 번만** 모아(chat_excluded) 동등 비교로 바꾼다.
-- 뜻은 그대로: "팬 챕터나 다시보기 제목이 이미 그 말을 잡은 방송은 채팅 행으로 또 내보내지 않는다".
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0117_public_search_chat_exclude.sql
...
as $$
  ...
  q as (
    select
      ...
      -- IDF 가중치는 **두 낱말 이상일 때만** 만든다(0116). 아래 쓰임새가 전부 `q.ntok > 1` 뒤에 있어
      -- 한 낱말 질의에서는 만들어 놓고 안 썼는데, 만드는 값이 말뭉치 18,824행 전수 스캔 두 번이었다.
      case when cardinality(q1.toks) > 1 then array(
        ...
        order by t.ord
      ) else array[]::numeric[] end as tokw,
  ...
  -- 그 말을 이미 잡은 방송(챕터 라벨·코너 또는 다시보기 제목) — 한 번만 모아 둔다(0117).
  -- 예전엔 채팅 후보 행마다 chm/vodm을 다시 훑어 중첩 반복이 됐다.
  chat_excluded as materialized (
    select c.title_no from chm c where c.hit_t or c.hit_s
    union
    select x.title_no from vodm x where x.hit_t
  ),
  ...
  chat_scored as (
    ...
    where q.min_ok and not q.is_cho and t.cnt >= 5 and t.peak_bin is not null
      ...
      and not exists (select 1 from chat_excluded e where e.title_no = t.title_no)
  ),
  merged as (
...
comment on function public.search_public(uuid, text, integer) is
  '시청자 검색(0076~0117) — 오타 교정 + 공연 종류·종겜 의도·챕터 코너·동의어/관련어/줄임말/IDF(두 낱말 이상)/초성, 낱말 경계 존중. 공개 데이터만.';
