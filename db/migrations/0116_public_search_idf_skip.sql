-- 0116: 시청자 검색(search_public)에서 **한 낱말 질의**가 말뭉치를 통째로 훑던 것을 없앤다.
--
-- 실측(EXPLAIN ANALYZE, '노래'): 전체 1,439ms 중 866ms가 tokw(IDF 가중치) 계산이었다.
-- tokw는 토큰마다 `select count(*) from corpus where corpus.norm ~ '(토큰)'`을 돌리는데, corpus는
-- 일정 582 + 다시보기 385 + 챕터 17,857 = 18,824행을 그때그때 이어 붙여 만드는 CTE다. 즉 검색 한 번에
-- 말뭉치 전수 스캔이 두 번(총 개수 + 토큰별) 들어간다.
--
-- 그런데 tokw가 쓰이는 자리는 전부 `q.ntok > 1`(= 두 낱말 이상)로 막혀 있다 — 한 낱말 질의('노래',
-- '토리', '와우' …)에서는 계산해 놓고 한 번도 안 쓴다. 그래서 **토큰이 둘 이상일 때만** 만든다.
-- CASE는 단락 평가라 한 낱말이면 corpus CTE 자체가 실행되지 않는다. 결과·순위는 그대로다.
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0116_public_search_idf_skip.sql
...
comment on function public.search_public(uuid, text, integer) is
  '시청자 검색(0076~0116) — 오타 교정 + 공연 종류·종겜 의도·챕터 코너·동의어/관련어/줄임말/IDF(두 낱말 이상)/초성, 낱말 경계 존중. 공개 데이터만.';
