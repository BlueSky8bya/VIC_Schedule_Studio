-- 0108: 검색어 제안 — 연구 브리프 반영(자동완성 후보는 문서 어휘만, 채팅 전용어는 관련 칩에서만) + 이름 굴절형 오인 수정('아마최'→'아마')
-- + 같은 사람의 다른 이름은 사용량(3방송↑) 상위 3개만 관련어로. 0107 재정의.
-- 근거: Bhatia et al. 2011(코퍼스 구 사전), Cai & de Rijke 2016(MPC+유형 슬롯), Algolia 제안 규칙(최소 빈도·정규화 병합).
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0108_public_search_suggest_research.sql
...
as $$
  ...
  direct0 as (
    ...
    where s.m > 0 and length(s.tn) >= 2 and s.tn <> q.qn and s.tn not in (select w from stop)
      ...
      and (s.kind <> 'term' or s.tn in (select term from public.search_doc_words))
      -- 굴절형 제거는 일반 단어에만(이름은 '아마최'→'아마' 같은 오인이 난다)
      and (s.kind <> 'term' or not exists (
