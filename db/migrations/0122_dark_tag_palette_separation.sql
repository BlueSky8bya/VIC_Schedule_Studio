-- Dark palette v2: stronger hue separation; original light colors and v1 remain.
-- Additive rollout: apply before the v2 readers. Rollback uses the v1 readers.
-- Exact formula shared with lib/tags/dark-palette.ts; immutable v1 is untouched.
begin;
set local lock_timeout = '5s';

create or replace function public.dark_tag_color_v2(
  source text, base_l double precision, l_gain double precision,
  chroma_gain double precision, chroma_cap double precision, chroma_offset double precision
...
begin
  ...
  chroma := least(chroma_cap,chroma*chroma_gain+least(chroma/0.04,1)*chroma_offset);
  high_c := chroma;
...
create or replace function public.dark_tag_palette_v2(source text)
returns jsonb language sql immutable strict parallel safe set search_path = pg_catalog as $$
  select jsonb_build_object('version',2,'source',case when source ~ '^#[0-9a-fA-F]{6}$' then lower(source) else '#b8b3aa' end,
    'bgColor',public.dark_tag_color_v2(source,0.35,0.12,0.95,0.14,0.035),
    'borderColor',public.dark_tag_color_v2(source,0.60,0.05,1.05,0.16,0.035),
    'textColor','#f1ede6',
    'accentColor',public.dark_tag_color_v2(source,0.74,0.06,1.0,0.16,0.025));
$$;

alter table public.color_palette add column if not exists dark_palette_v2 jsonb
  generated always as (public.dark_tag_palette_v2(bg_color)) stored;
alter table public.broadcast_tags add column if not exists dark_palette_v2 jsonb
  generated always as (public.dark_tag_palette_v2(bg_hex)) stored;

revoke all on function public.dark_tag_color_v2(text,double precision,double precision,double precision,double precision,double precision) from public;
revoke all on function public.dark_tag_palette_v2(text) from public;
grant execute on function public.dark_tag_color_v2(text,double precision,double precision,double precision,double precision,double precision) to anon, authenticated, service_role;
grant execute on function public.dark_tag_palette_v2(text) to anon, authenticated, service_role;
