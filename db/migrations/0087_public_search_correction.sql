-- 0087: 시청자 검색 — 한글 오타 자동 교정을 검색 RPC에 배선(0086 search_correct).
-- 질의가 어떤 사전(동의어·의도·인물·게임)에도 없고 초성·한 글자·다단어가 아니면 가장 가까운 사전어로
-- 바꿔 찾고(노레→노래, 챌린쥐→챌린지), 반환 행마다 corrected(교정어, 없으면 '')를 붙인다. 로더가 이를
-- 응답 corrected로 올리고 시트가 "'노래'로 찾았어요"를 보인다. 영타(shfo)는 로더가 두벌식 변환.
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0087_public_search_correction.sql
...
comment on function public.search_public(uuid, text, integer) is
  '시청자 검색(0076~0087) — 오타 교정(자모 편집거리) + 공연 종류·종겜 의도·챕터 코너·동의어/관련어/줄임말/IDF/초성. 공개 데이터만.';
