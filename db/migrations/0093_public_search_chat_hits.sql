-- 0093: 채팅 단어 → 검색 적중(반응 구간으로 점프) + 단어별 봉우리 구간.
--
-- 소유자(2026-09-18): "채팅에서 탐정이 여러 번 나오면 수집해 뒀다가 '탐정' 검색 때 거기로 갈 수 있게".
--  · vod_chat_terms에 peak_bin(그 말이 가장 몰린 30초 구간)·peak_cnt·bins(나온 구간 수)를 둔다. 수집기가 채운다.
--  · search_public에 chat_scored: 단어 전체 일치·5회 이상·팬 챕터가 못 잡은 방송만, 챕터 모양 행(section '채팅', matched_on 'chat').
--  · 별명 변형(탐토리·탐정토리·바보토리·빅드럭…)은 수집기가 어간을 풀어 센다(vod-chat.ts stems).
-- 앞서 받은 2개 방송은 봉우리가 없어 다시 받는다.
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0093_public_search_chat_hits.sql

alter table public.vod_chat_terms add column if not exists peak_bin integer;
alter table public.vod_chat_terms add column if not exists peak_cnt integer not null default 0;
alter table public.vod_chat_terms add column if not exists bins integer not null default 0;
delete from public.vod_chat_sync where title_no in (select title_no from public.vod_chat_terms where peak_bin is null);
delete from public.vod_chat_bins where title_no in (select title_no from public.vod_chat_terms where peak_bin is null);
delete from public.vod_chat_people where title_no in (select title_no from public.vod_chat_terms where peak_bin is null);
delete from public.vod_chat_terms where peak_bin is null;
...
comment on function public.search_public(uuid, text, integer) is
  '시청자 검색(0076~0093) — 오타 교정(자모 편집거리) + 공연 종류·종겜 의도·챕터 코너·동의어/관련어/줄임말/IDF/초성. 공개 데이터만.';
