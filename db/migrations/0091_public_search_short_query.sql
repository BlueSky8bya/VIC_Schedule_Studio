-- 0091: 검색 품질 — 두 글자 질의의 원문 경계 + 요즘 칩은 말뭉치·사전에 있는 말만.
--
-- 소유자(2026-09-18):
--  · "이게"를 검색했더니 "이 게임"이 걸렸다 — 정규화(띄어쓰기 제거) 때문. 두 글자 한글 질의(사전 확장 없음)는
--    제목·설명·태그·챕터 **원문**에서 붙어 있을 때만 적중. 퍼지(유사도)도 두 글자엔 끈다.
--  · 요즘 칩에 "아니야" 같은 말이 떠서 눌러 봐야 일정이 없다 — 채팅에서 온 말은 제목·챕터·일정 말뭉치나 사전
--    (동의어·인물·게임·의도)에 있는 것만 칩으로 올린다 = 누르면 반드시 결과가 있다.
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0091_public_search_short_query.sql
...
comment on function public.search_public(uuid, text, integer) is
  '시청자 검색(0076~0091) — 오타 교정(자모 편집거리) + 공연 종류·종겜 의도·챕터 코너·동의어/관련어/줄임말/IDF/초성. 공개 데이터만.';
...
as $$
  ...
  words as (
    ...
    where t.term ~ '^[가-힣]{2,6}$' and t.term in (select w from known)
  ),
  stop as (
