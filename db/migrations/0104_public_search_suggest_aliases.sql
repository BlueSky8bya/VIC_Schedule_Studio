-- 0104: 검색어 제안 — 스트리머 별칭(seed-person·seed-web)도 앞글자 제안에 포함(사람 표시) + 관련어 불용어 보강(몰라·논란·토리도…).
-- 소유자(2026-09-18): "별명으로 검색해도 관련어에 해당 스트리머가 나와야". 0099 재정의.
-- 멱등. 적용: node scripts/apply-db.mjs db/migrations/0104_public_search_suggest_aliases.sql
...
-- 관련 인물(0090 재정의): 질의→인물 해석에 **동의어(syn)만** 쓴다. auto-rel(공출현 칩)까지 쓰니 '마최'가 샬롯으로 풀렸다(실측).
...
as $$
  ...
  me as (
    ...
    where e.name = q.qn or e.name || '님' = q.qn
       or e.name in (select s.alt from public.search_synonyms s where s.term = q.qn and s.kind = 'syn')
    order by (e.name = q.qn) desc, e.docs desc
    limit 1
  )
  select
