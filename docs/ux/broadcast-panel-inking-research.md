...
> 2026-07-26 작성·2026-07-27 갱신·감사. 판서 패널 개선의 근거 문서다. 코드를 고치기 전에 먼저 갱신한다.
...
- Pointer Events는 `pointerType:"pen"`으로 스타일러스 입력을 구분하고, hover를 지원하는 pen이
  감지 범위를 벗어나면 `pointerleave`가 발생한다고 정의한다. CSS `cursor`는 pointing device의
  커서를 지정하지만, 운영 기기에서는 pen 접촉 중 시스템 커서가 표시되지 않는 사례가 확인됐다.
  이는 잉크 입력 실패가 아니라 네이티브 커서 표시에만 의존한 제품 결함이다.
...
- 마우스에는 기존 CSS 도구 커서를 유지하고, `pointerType:"pen"`에는 board 좌표면 위 DOM
  커서를 별도로 표시한다. hover·down·move에서 펜촉을 따라가며 up·cancel·capture 상실·leave에서
  숨긴다. 펜·형광펜·지우개는 굵기 관계를 반영한 footprint, 도형은 crosshair와 도구 아이콘을 쓴다.
  고빈도 move는 React state가 아니라 ref의 compositor transform만 갱신한다.
...
- 패널을 새로 열면 `펜` + 검정 잉크 + 표시·잠금 해제된 `레이어 1`을 활성화해 추가 선택 없이
  바로 그린다. 서버·브라우저 저장을 하지 않는 일회성 패널 계약상 닫았다 다시 여는 것은 새 판서
  세션이다. 첫 일정 보내기만 기존 규칙대로 `일정` + `선택` 문맥으로 전환한다.
