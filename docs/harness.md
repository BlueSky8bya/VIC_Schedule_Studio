...
Drift is advisory: a matching domain record or exact-content acknowledgement satisfies the mechanical check, but cannot prove that the explanation is semantically correct. CURRENT_STATE alone is not a substitute for a changed domain contract. See [repair evidence](agent/handoffs/20260909-memory-and-art-pipeline-repair.md) and [current work](agent/CURRENT_STATE.md).
