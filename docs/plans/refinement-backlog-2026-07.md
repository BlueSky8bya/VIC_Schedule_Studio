...
> **상태**: ✅ **배치 1~10 전부 완료(2026-07-17)**. 아래 항목의 ✅ 표시와 커밋을 참고.
> 남은 보류: C8(공개 로더 ±13개월 윈도 — 동작 변경 소지) · C5(perf 표본 샘플링 — 저트래픽이라
> 이득 < 패널 정확도 손실) · D12(시트 퇴장 모션) · D13(jumpToday 360ms 타이머) ·
> 인사이트 준비확인 쿼리(fetchAllRows가 에러를 삼켜 없애면 권한오류/데이터없음 구분 불가).
>
> **이 감사에서 배운 것(다음 감사에 적용)**
> 1. **수치 없는 "O(N²)처럼 보인다"를 최우선에 올리지 말 것** — C1은 실측하니 1.1배였다(filter가
>    sort보다 먼저 돌아 N²이 안 터진다). 정작 큰 이득은 C9(리사이즈 헛 리렌더)·C4(번들 -25kB)였다.
> 2. **감사 목록을 그대로 믿고 지우지 말 것** — `.notice-method`(11곳 사용), `/api/public/.../events`
>    (공개 API 계약)는 "죽었다"고 적혀 있었지만 살아 있었다. 지우기 직전 재확인이 두 번 다 살렸다.
> 3. **코드 리뷰로는 못 보는 게 있다** — 하트 실패가 throw일 때 롤백조차 안 되던 것, 미디어쿼리
>    우선순위, popstate 리스너 호출 순서는 전부 '실물 테스트'가 잡았다.
...
### ✅ C1. `getEventsForDate` — 2026-07-17 완료. **단, 아래 "최우선" 판단은 실측으로 틀린 것으로 판명**
> **정정(중요)**: 감사는 "42칸 × 500 × 9비교 × 500스캔 ≈ 9천만 연산/렌더"라고 했지만 **틀렸다**.
> `filter`가 `sort`보다 먼저 돌아, 비교자는 *그 날에 걸친 몇 개*만 정렬한다 → N²이 안 터진다.
> 실측(옛 구현 vs 새 구현, 렌더 1회 = 42칸):
> | 일정 수 | 옛 | 새 | 배수 |
> |---|---|---|---|
> | 100 | 0.09ms | 0.08ms | 1.0× |
> | 250(현재 실데이터 240) | 0.22ms | 0.20ms | **1.1×** |
> | 500 | 0.72ms | 0.44ms | 1.6× |
> | 1000 | 0.98ms | 0.86ms | 1.1× |
> 즉 **여긴 병목이 아니었다.** 리팩터는 유지(더 빠르고 의미가 또렷하며 순서 규칙은 동일 — 옛/새
> 결과가 10~500개 × 42칸에서 완전 일치 확인, `tests/unit/events-for-date.test.ts` 10개가 규칙을 고정).
> **교훈: "O(N²)처럼 보인다"를 수치 없이 최우선으로 올리지 말 것.** 아래 원문은 기록용.
...
### ✅ C9. ResizeObserver가 스로틀 없이 4,400줄 트리를 리렌더 — 2026-07-17 완료
> rAF로 프레임당 1회 + 값이 그대로면 setState 생략. 실측: 폭이 바뀌는 리사이즈 26단계 → 갱신
> 24회(정상 추종, 배율은 폭을 그대로 따라감), **폭이 안 바뀌는 흔들기 6회 → 갱신 0회**(예전엔
> 콜백마다 리렌더). 이쪽이 C1보다 체감 이득이 크다(창 끌 때 프레임 낭비 제거).
...
### ✅ D1. 모바일·태블릿에서 '이 달 기록'을 **아예 못 연다** (ISSUE-002) — 2026-07-17 완료
> 아젠다 범례 레일 아래에 같은 버튼(92×44). 실측: 폰 390·태블릿 900 모두 버튼 1개 보임(예전 0개)
> → 탭하면 시트 열림. 웹 1600은 레일이 없어 중복 없음(헤더 1개).
...
### ✅ D4. 필터 누르면 달력이 **뚝** 끊긴다 (트랜지션 누락) · W:중 E:S — 2026-07-17 완료
> 사용자 요청으로 앞당겨 처리. 덤으로 **업 도움 끈(`.support-bar`)도 필터에 같이 물러나게** 했다 —
> 카드만 흐려지고 끈은 쨍하게 남아, 안 고른 기간이 오히려 제일 눈에 띄었다. 끈 본체 CSS는
> 편집실과 공용(`app/globals.css`)이라 흐림·전환은 `.public-day` 안으로 한정했다.
...
### ✅ D5. 하트 탭 타깃이 18×18px — 2026-07-17 완료
> 보이는 하트 18px 유지 + 투명 `::after`(inset −13px)로 누름 범위만 44×44. 실측으로 표면 높이
> 1316.8·첫 카드 좌표가 그대로임을 확인(상자를 키웠으면 카드 줄바꿈이 변해 스티커가 밀렸다).
...
### ✅ D6. 모바일 필터 칩이 정적 + 27px — 2026-07-17 완료
> `:active` 축소 + 상태 전환 추가, 타깃은 `::after` 패드(레일 92px·높이 제약이라 칩 자체는 못 키움).
> 실측: 칩 위 6px 지점을 눌러도 칩이 잡힌다.
...
### ✅ D7. `.legend-clear` / `.agenda-legend-clear` 정적 — 2026-07-17 완료
...
### ✅ D10. `.mb-step` 48px vs `.mb-act` 40px — 2026-07-17 완료(`.mb-act` 44px로)
`css:406-410` vs `:452`. "형제 높이 불일치 = 결함"이고 40px는 44px 미달. → `.mb-act { min-height: 44px }`.

### ✅ D11. 인사이트 시트에 모바일 재단이 **전혀** 없다 — 2026-07-17 완료
> 폰(≤640px)은 바텀시트(바닥에 붙고 아래에서 올라옴) + 통계 2열. 실측: 폰 390에서 시트 390×743이
> 바닥에 붙음·통계 2열·가로 넘침 0. 태블릿·웹은 기존 가운데 카드 유지.
> **함정**: 미디어쿼리는 우선순위를 안 올린다 → 모바일 블록을 `.pi-*` 기본 규칙보다 **앞**에 두면
> 그대로 덮인다(실제로 통계가 계속 3열로 나와 발견). 블록은 섹션 끝에 둘 것.
