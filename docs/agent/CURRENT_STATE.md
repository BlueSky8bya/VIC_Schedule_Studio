...
- **▶ 다음에 할 일 — 갈래 둘, 서로 독립이다(2026-09-08).**
  **ⓐ 아트: 파일럿 잔여 11장.** `/studio/ambient-art`(개발자) → 필터 **파일럿** → **「남은 파일만」** → 프롬프트 복사 → 코덱스.
  목록: `tree-pine-2` · `tree-pine-autumn-2` · `tree-pine-winter-2` · `shrub-autumn-1` · `reed-1,2` · `rock-1~4` · `sea-stack-1`.
  **바위 4장 먼저** — 사철 열한 바이옴에 다 깔리고, 격자 16칸/블록 64px로 도트가 가장 굵어 규격 위반이 제일 잘 보이며,
  변형이 4개라 '복제 금지'가 처음 검증된다. 받으면 `public/ambient/art/`에 두고 `npm run art:normalize`(원본은 `art-src/`에 자동 보관) → 게이트.
  그다음 순서: 정체성 초목 변형 13장(참나무·관목·자작나무·그루터기·통나무·묘목의 둘째 변형) → **도감 1차 43종(65장)** → 2차 → 3차.
  **하늘 8자리 26장은 끝났다** — 「남은 파일만」이 다시 묻지 않는다.
  **ⓑ QA 라운드 17: 비의 젖음(W-1 ③).** 아래 라운드 17 항목 참조. 아트와 병행 가능하다.
...
- **✅ 도감 세 권 + 아트 보드 대개편(2026-09-07, PLAN-20260907-008 · ADR-0019)** — 소유자 지시: "동물의 숲처럼 도감 수집하는 맛. 동물도 우리가 새로 그린다. 식생·돌·구름·천체도 여러 버전이 있어야 한다."
  ...
  **다음 생성 순서**: 파일럿 잔여 11장 → **천체·구름 26장**(달 위상이 날짜의 표정이 된다) → 참나무·관목 변형 8장 → 도감 1차 43종 → 2차 → 3차. 얇은 바이옴(산 10 · 암석해안 9 · 깊은 바다 7 · 먼바다 1차 1종)은 2차 배치에서 먼저 채운다.

- **(위 ▶ ⓐ의 근거 · 2026-09-07) 아트: 파일럿 잔여 11장** — `/studio/ambient-art`의 **「남은 파일만」** 버튼이 프롬프트를 만든다(자리가 아니라 **파일** 단위로 걸러 이미 합격한 그림을 다시 부탁하지 않는다). 목록: `tree-pine-2` · `tree-pine-autumn-2` · `tree-pine-winter-2` · `shrub-autumn` · `reed-1,2` · `rock-1~4` · `sea-stack-1`. 권고 순서 = **바위 4장 먼저**(사철 전 바이옴에 깔리고, 격자 16칸/블록 64px로 파일럿 중 도트가 가장 굵어 규격 위반이 제일 잘 보이며, 변형 4개라 '복제 금지' 규칙이 처음 검증된다) → 통과 후 나머지 7장.
