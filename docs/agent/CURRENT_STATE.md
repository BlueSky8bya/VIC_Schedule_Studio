...
- **▶ 아트: 다음 생성 = 파일럿 잔여 11장(2026-09-07)** — `/studio/ambient-art`의 **「남은 파일만」** 버튼이 프롬프트를 만든다(자리가 아니라 **파일** 단위로 걸러 이미 합격한 그림을 다시 부탁하지 않는다). 목록: `tree-pine-2` · `tree-pine-autumn-2` · `tree-pine-winter-2` · `shrub-autumn` · `reed-1,2` · `rock-1~4` · `sea-stack-1`. 권고 순서 = **바위 4장 먼저**(사철 전 바이옴에 깔리고, 격자 16칸/블록 64px로 파일럿 중 도트가 가장 굵어 규격 위반이 제일 잘 보이며, 변형 4개라 '복제 금지' 규칙이 처음 검증된다) → 통과 후 나머지 7장.
...
- **▶ 다음에 할 일 = 코덱스에게 소나무 3장을 뽑아 화풍 판정(2026-09-07, 결정 1b 닫힘)** — 막혀 있던 소유자 결정이 **ⓐ′**로 정해졌다(보간 끄기 + 정수배 축소 + 자리별 도트 격자). 파이프라인·프롬프트·보드가 그에 맞게 고쳐졌으니 **이제 생성만 남았다.**
