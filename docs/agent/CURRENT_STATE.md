...
- **앰비언트 비주얼 QA 라운드 14 완료(2026-09-06, `docs/ambient/rounds/ROUND-14.md`)** — 자율 6라운드의 다섯 번째, 우선순위 B **날씨 반응**. 세 검사관 한 뿌리("날씨가 그림을 바꾸지 않고 색만 바꾼다" = "`Light`가 세기만 주고 면이 날씨를 안 읽는다"): 전역 ΔL을 빼면 비 잔차 0.00~0.11%·바람 0.00%, 비 고리는 민물뿐, 물보라는 `shoreY−4` 고정선(썰물엔 x의 63%가 마른 뭍에서 솟음), 바람 방향이 한 화면에 둘. ① **라운드 13 회귀 복구**: `bakeShoreRocks`가 `resize()`에만 있어 창 크기가 그대로면 층이 마운트 조석에 굳었다(점심에 연 화면이 밤이 되어도 점심 잠김 유지 — bbox 16.4% 차) → `step()`에서 조석 상태가 바뀔 때만 굽는다 ② **물 = 날씨의 소비자**: `scenes/water.ts`에 공용 `RainRing`·`stepRainRings`·`drawRainRings`(민물의 고리를 옮김) → 해안은 물가선 `lineY(x)` 위, 먼바다는 수평선 아래, 원근 가중 `r^0.55`(고른 산포 금지); 물보라 앵커 = 이 조석에서 **물속에 발을 담근 `shoreRocks`**, 없으면 물가선(바위만 보면 밤 썰물에 물보라가 0); `particles.ts windDirOf(seed)` → `Frame.windDir`로 **바람 부호 공유**(물보라 vx·거품 마디 흐름·파도 α); 거품 마디는 바람에 길이 ×(1+.5w)·틈 ×(1−.3w)·굵기 ×(1+.35w)로 반응(흰 α를 올리면 형광펜) ③ `light.ts` 저녁·밤 tint α .10 → .04(tint는 source-over라 무채색에 크로마를 만든다 — 뻘 C* ×2.82). 실측: 비 고리 암석 17·갯벌 26·먼바다 20·민물 16(맑음 0) · 물보라 밤 썰물에도 20 · `rocksTide` null → 0/−1/1 · 밤 하한 민물 −20.1 → **−14.8**(모래 −16.7 경계) · 비해안 시나리오는 저녁·밤 프레임만 변화 · 회귀 전부 유지(tuft 0/16/0 · 깊은 바다 해시 6 · 다람쥐 8/8 · 안개 D 5/7) · selftest ALL PASSED · tsc/lint/vitest(604)/build 0. 이월(라운드 15): 수평선 반사 띠 × 날씨(비 peak−sky 17.4) · 입자 픽셀 규격(빗줄기·눈송이 한 크기) · **땅의 젖음·바람 lean** · 겨울 해안 눈 조간대 · 젖은 자리 돌 +3.9L · 무채색 크로마 잔여 · 계곡 유로 고리. 다음 = **라운드 15**(마무리 묶음 F~I).
