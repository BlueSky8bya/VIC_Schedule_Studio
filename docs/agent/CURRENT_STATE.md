...
- **편집 카드 = 앵커 팝오버 전환(2026-07-31, 사용자 결정 — 목업 승인 후)**: 데스크톱 편집
  ...
  6차 스타일(사용자 요청): 카드 반투명(배경 80%/내부 컨테이너 55% — 입력·버튼·칩은 불투명)
  + 카드 밖 4px 오프셋 모드색 점선 아웃라인(리더 라인과 같은 색 언어) + 앵커 도트 흐림
  (r4.5, fill-opacity .42, 흰 테두리 — 일정 카드 글자 안 가림)(`beac3d6`). 7차 태그 트레이
