...
- **바이옴 44장 화질 검토 5회전(2026-09-04 밤, ADR-0017 ⑰)**: 계절 4 × 화면 11 = 44장을 찍어
  **현실성 · 경계 마감 · 미관** 세 관점으로 다섯 번 돌리고 매 회전마다 고쳤다. 하네스는
  `.scratch-pw/snap-biomes.mjs <사이클>`(감상 진입 → 열한 화면 순회 → PNG 44장).
  큰 것: ① `scatterProps`가 아트 없는 자리를 전부 건너뛰어 바위·그루터기·통나무·관목·갈대가 **어느 바이옴에도
  없었다**(대체물 화가 11종 추가, 픽셀 격자로 굽기) ② 안개가 지평선에서 곧장 켜져 44장 전부에 가로 절단선
  (지평선 위에서 0으로 시작하도록) ③ 감상 중 오른쪽 15px 종이색 띠(`scrollbar-gutter`) ④ 산이 회색 막대 두 개
  → 각진 능선·명암 면·만년설·침엽수·너덜 ⑤ 계곡 벽·물가 바위·시내 리본 ⑥ 해안 셋의 계절 팔레트·만곡 물가·
  젖은 띠·물골 방향(관찰자 → 바다)·조수 통일 ⑦ 물: caustic은 얕은 물에만·채운 셀·수평선 블렌드·파장 원근
  ⑧ 깊은 바다에서 수면 문법 제거 + 바다눈·광선 ⑨ 가을 팔레트 위반(벽돌빛 빨강·금색 낙엽) 시정 ⑩ 겨울 눈밭
  명암 폭 9 → 43 ⑪ 세로 이동 크로스페이드. 실측(σ, 지평선 아래 프레임): 초원 10.9 → 17.0 · 언덕 → 20.0 ·
  가을 초원 → 23.4 · 갯벌 → 27.0 · 깊은 바다 → 33.3(숲 40.6이 기준선). 검증: tsc 0 · lint 0 · vitest 555 ·
  build exit 0 · `probe-biomes`(4계절 23항목) · `probe-ground` · `probe-art-board` 전부 통과.
  **남은 것**: `rock` 아트 4종(9개 바이옴에 등장) → 관목 4계절 → 갈대·통나무·그루터기 → 눈 무더기.
  연못·먼바다·모래해안은 아직 화면 절반이 빈 물/모래 판이다(P2 생물이 채운다).
