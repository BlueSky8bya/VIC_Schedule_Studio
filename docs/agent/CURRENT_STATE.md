...
- **편집실 배치 대개편 + 물결 레이어 + 동작 스위치 극성(2026-09-03, PLAN-20260903-002)**:
  ...
  §6 WorldScene(엔진 최소 변경, 활성 화면만 step, summer.ts 물 코드는 pond/shore로 이사), §7 로드맵(P0 축척 → P1 세계·카메라 → P2 에이전트
  → P3 아트 → P4 도감), §8 소유자 결정 5건(여름 기본 = 초원 · 십자 5 · 오행 배치 · 입력 전부 · 시청자도). **P0은 결정 없이 바로 가능.**
