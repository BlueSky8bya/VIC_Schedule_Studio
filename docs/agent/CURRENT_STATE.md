...
- **▶ 다음 세션 진입점 — 소나무 21장 재작업(2026-09-08, 다른 기기에서 이어받을 수 있게 기록함)**
  **인계 문서: [docs/ambient/HANDOFF-20260908-pine.md](../ambient/HANDOFF-20260908-pine.md)** — 상태·반려 사유 실측·다음 할 일·프롬프트 다시 뽑는 법이 거기 다 있다.
  한 줄 요약: 코덱스가 소나무 21장을 보냈고 **반려**다. 반려본 24장(21 + 코덱스가 덮어쓴 `-1` 3장)은 `art-src/incoming-pine/`에
  **커밋돼 있고**, `public/ambient/art/`의 소나무는 **합격본 `-1` 세 장뿐**이다(반려본이 화면에 깔리지 않게 되돌렸다).
  ✅ 짝은 성공 — `-n`의 계절 실루엣 일치 봄↔가을 **100%** · 봄↔겨울 **99%**(프롬프트가 짝을 못 박은 것이 먹혔다).
  ❌ 반려 셋 — ① 잎이 **잘게 부서진 스티플**이라 합격본(`-1`)의 굵은 블록과 어법이 갈린다 ② 겨울 눈이 **얇은 흰 줄**이라
  실루엣을 안 바꾼다(브리프는 "단 윗면에 두툼하게") ③ 여덟의 실루엣 유사도가 **74~91%**로 덜 갈렸다(`-4`↔`-7` 91%).
  ⚠ 코덱스가 합격본 `-1`을 덮어썼다 — 보관 후 되돌렸다. **덮어쓰기를 보면 먼저 보관하고 되돌린다**(2026-09-07에 반대 사고가 있었다).
