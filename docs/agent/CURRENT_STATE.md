...
- **ISSUE-007 — 비주얼 스냅샷 6건이 기준선 드리프트로 red(오늘 작업과 무관, 확인됨).**
  `geometry.spec`(표면 h 1137→998) + poster/redesign-matrix 스크린샷 5건. 2026-08-05 작업 전
  커밋(`4c3a6a6`)을 별도 worktree로 띄워 돌려도 **똑같이 실패**한다 → 오늘 변경이 원인이 아니다.
  기준선을 그냥 갱신하면 표면 높이 139px 축소가 언제·왜 생겼는지 증거가 사라진다(스티커는 비율
  좌표라 ADR-0004 불변식과 직결). 원인 커밋을 먼저 찾을 것. Status: Open.
