...
- **앰비언트 비주얼 QA 라운드 12 완료(2026-09-06, `docs/ambient/rounds/ROUND-12.md`)** — 자율 6라운드의 세 번째, 우선순위 C **간격·레이어**. 세 Inspector 한 뿌리("소품 필드가 세로 실루엣과 풀 층을 모른다"). ① `art/props.ts claimSpot(x, y, r, stand, hy)` — 실루엣 높이 규칙(같은 열에서 뒤 것의 발이 앞 것의 몸통 사각 안이면 거부; 옛 규칙은 수관 반지름만 봐서 바위 쌍·소나무 4그루 적재 통과) + `behindStand(x, y)` + `scatterProps` stand 자동 등록 · 후보 y 정렬 ② 미경유 7곳 경유(산 큰 바위 · 계곡 수변 · 계곡 벽 나무 · 민물 기슭/섬/앵커 바위 · 초원 관목 · 언덕 능선 나무) + 나무 hy(소나무 3.6R · 참나무 2.5R) + 풀포기 스킵(`tuftAt`·계곡·초원 `blades`) + `summer.ts` 필드 리셋 + `debug().spots` — **B 판정: 풀 층은 옮기지 않는다**(관통 획은 전부 뒤 뿌리, 앞 뿌리는 정상 가림; `standing[]` y-sort는 P2 단계로) ③ **라운드 11 회귀 셋**(C): `depthFade` 안개항이 alpha라 먼 나무가 투명(유령, α̂ 물체/지면비 계곡 2.99) → 제거 · 평지 `fogFloor`가 `f.h`를 돌려줘 숲 밀도장 12% → NaN + `isFinite` 가드 · **`world-scene.ts`가 `fogFloor`/`fogFloorKey`를 위임하지 않아 라운드 11의 다섯 지면선이 화면에 도달한 적 없음**(숲 = 초원 α̂ 동일) → 위임(전이 중 NaN); 배선 뒤 계곡 중경 2.89 → 1.29, 언덕 능선 위 D 5.95 < 아래 7.57 ④ 안개 halo α .16 → .35. 실측: 깊은 바다 아침→점심 채널 0 → 3 · 밤 글로우 1.90 ≥ 1.4 · hz 절단선 닫힘 유지 · 회귀 0 · selftest ALL PASSED · tsc/lint/vitest(604)/build 0. 이월: 13 D 해안 뭍 클립(`lateRocks`) · 14 날씨 지면/수면/그림자 + 바람 + tint + 안개 물 반사·뭉치·수평선 · 15 선반 y 분포·쌍둥이 바위·초원 소나무 머리·겨울 sky↔①·잠긴 바위 한 개. 다음 = **라운드 13**.
