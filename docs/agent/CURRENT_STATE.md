...
- **▶ 다음 세션 진입점(2026-09-07 소유자 관찰, 아직 수정 안 함)** — 가을 화면에서 **레이어 두 건**. 상세·코드 경로·고칠 방향은 [docs/ambient/QA_PROGRESS.md §3.5](../ambient/QA_PROGRESS.md).
  ① **AMB-D3-04(P1)** 안개가 **물체의 거리가 아니라 화면 높이**로 걸린다 — 엔진 `view.ts drawDepthHaze`가 장면·입자 위에 한 겹(`scene-engine.ts` 436→443), 거기에 `autumn.ts`의 `mist`(위 34%)가 또 한 겹. 그래서 어떤 높이 위는 나무든 바위든 같이 하얘진다.
  ② **AMB-A3-02(P1)** 낙엽·도토리가 **모든 나무 앞**에 그려진다 — 나무는 `bakeGround`에서 바탕에 구워지고 잎은 마지막에 한 덩어리로 그려져 y-sort가 없다. 봄(나비·꽃잎)·겨울(눈·발자국)도 같은 구조.
  제안: 바탕을 지면/나무 두 장으로 굽고 그 사이에 입자를 끼우거나, 나무를 y-sort 대열로 옮긴다.
