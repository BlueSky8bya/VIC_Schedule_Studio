...
- **앰비언트 비주얼 QA 라운드 13 완료(2026-09-06, `docs/ambient/rounds/ROUND-13.md`)** — 자율 6라운드의 네 번째, 우선순위 D **해안 접촉**. 세 Inspector 첫째가 같은 것: 뭍 판이 물가 폴리곤으로 클립되어 물가 띠 바위의 꼭대기가 물가선에서 잘리고(수면선 위 바위 px 0), 거품 줄·흰 원반·조류대가 몸통 위, 조석에 수면선 고정, 뒤 노두가 앞 바위를 덮음 = "반투명 사다리꼴 판". `scenes/coast.ts`: ① **꼭대기(발 − 30k)가 밀물선(60 + .04h + 6) 위인 바위는 전부 판 밖 층** `shoreRocks`(암석 노두 + 큰 뭍 바위 + 갯벌 바위; 옛 초안 `발 y < 110`은 k 2.7~3 셋을 놓쳤다 — B) ② `claimSpot(…, true, 30k)` 실루엣 규칙 ③ **`bakeShoreRocks(tide)`** 조석별 두 층 — 발이 그 조석의 물속이면 `rocksSea`(잠김 `clamp(6 + .5·잠긴 폭, 4, 18k)` + 링 + 2px 거품 조각 3, **거품 줄 앞**), 뭍 위면 `rocksLand`(마른 돌 + 그림자, **거품 줄 뒤**); y 정렬, 클립·전단 없음, 흰 원반 삭제(A), `debug().shoreRocks/spots/rocksTide` ④ C #4: 거품 마디 시드 `round(sy)`(≈ 1회/초 재추첨, 40ms diff ×11) → 정적 시드 + 주기 ≥ 1100px 타일 4px/s 드리프트; 조석 `tideK` τ 3s(첫 프레임 = 목표값 → 캡처 결정성 유지). 실측: 물가 띠 바위 23/23 꼭대기 − 물가선 ≤ −8 · 수면선 위 px 0 → 1,900~2,600 · 뭍 쪽 몸통 안 거품 0 · 실루엣 교차 2 → 0 · diff max/med ×11 → 1.8 · 띠 전환 점프 17 → 2~4px · 해안 밖 13 시나리오 픽셀 동일 · selftest 통과 · tsc/lint/vitest(604)/build 0. 되돌림: `drawArt` nearest(rock 아트 없음). 라운드 12 C 보충: 바람 weatherDelta 10/11 ✘ 불변 · 새벽→아침 평면 잔차 하락(T-1 P2 신규). 이월: 14 B 수면(비 고리 공용 · 파도/거품 × wind · 물보라 = `shoreRocks` · 스워시) + 수평선 반사 띠 × 날씨 + tint α .04 + 안개 발/수관 + 계곡 우측 −11L · 15 갯골 하늘 반사 · 암석 뭍 "사구 판" · 민물 37/49 · 겨울 sky↔①. 다음 = **라운드 14**.
