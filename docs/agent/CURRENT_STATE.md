...
- **편집실 배치 대개편 + 물결 레이어 + 동작 스위치 극성(2026-09-03, PLAN-20260903-002)**:
  ...
  **보류(사용자 제안 1·2 — 평가만)**: 계절 배경 4종(사주 보정: 봄=木 초목, 여름=水 물결(현재), 가을=金 서리·
  채도 낮춘 낙엽, 겨울=水 눈·습기) + 특정일(성탄·할로윈·24절기) 앰비언트 레이어 레지스트리 —
  `docs/ux/seasonal-ambient-plan.md`에 설계안. 기존 포스터 테마 7종(`data-poster-theme`, 시청자 전용·정적)은 이
  레지스트리로 supersede 예정(ADR 필요).
