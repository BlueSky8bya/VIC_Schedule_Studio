...
Status: **새 봄 초원 3레이어 로컬 적용, 시각 피드백 대기** · Task Risk: **L2** (렌더 순서·좌표·아트 규격의 구조 변경) · Created: 2026-09-10 (KST)

현재 구현: [ADR-0024](../decisions/ADR-0024-spring-meadow-three-layers.md)의 새 지형 세 장과 봄 전용 .35 지평선. 시안08 자체는 제품에 사용하지 않았다. 현재 변경은 로컬이며 아래의 이전 푸시 승인은 완료된 ROUND-20 배포 기록이다.

2026-09-11 후속 피드백: 소유자가 더 멀고 높은 시점에서 현재 화면 2×2에 해당하는 땅을 한 화면에 담도록 명확히 요청했다. 아래의 “시점 유지”는 이 요청으로 대체한다. [ROUND-21](../../ambient/rounds/ROUND-21-meadow-camera.md)이 현재 구도·축척 작업과 적용 여부를 소유한다. 픽셀 색감·열린 중앙, 모바일 제외·감상 중 움직임·자동조절·심해 봉인은 유지한다.
...
P0 실행 상태·실제 검증: [ROUND-18-depth-P0](../../ambient/rounds/ROUND-18-depth-P0.md). 아래의 “현행/제안/아직 미구현”은 설계 확정 당시 조사 기록이며, 이후 실행된 인터페이스와 규칙 대체는 [ADR-0022](../decisions/ADR-0022-world-depth-and-showcase-motion.md)가 소유한다. 최초의 계획서 두 파일 제한은 P0 구현 지시로 코드·검증·관련 문서에 한해 대체됐다. P0 당시 이미지 생성은 0이며, 이후 봄 초원 생성·적용은 위 ROUND-20에 기록한다.
