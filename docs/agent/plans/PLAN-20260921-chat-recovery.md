...
Status: deployed and target restored. Migration0126 applied successfully with verified TLS; code33000e6c verified in production before repair, with an additional60s drain for old in-flight collectors.
...
Production evidence: VOD207499291 now returns a nonempty, ratio-only public profile. The actual `/replay/2026-09-18?part=2` page rendered its SVG reaction waveform; screenshot inspected. Three upstream chunks still return unavailable responses and remain scheduled gaps, not false completion. Manual invocation of the same recurring GitHub workflow succeeded ([run35611465488](https://github.com/BlueSky8bya/VIC_Schedule_Studio/actions/runs/35611465488)); its protected API reported successful checkpointing for3VODs/120source chunks. No real test fixtures or raw chat were persisted.

Limits: unavailable upstream portions cannot be reconstructed; publish valid portions and retry gaps up to daily. Speaker density at chunk/file boundaries uses summed anonymous counts rather than retained identities. Scheduler timing depends on GitHub Actions; no instant-update guarantee. Existing published profiles stay in place until the new pass traverses the entire source plan. Historical archives rebuild progressively through the fair queue, not in one deployment.
