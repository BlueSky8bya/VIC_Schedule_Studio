...
## ⑱ 캔버스 화질 규칙 — 라운드2 5회전에서 굳은 것(2026-09-04 밤)

44장을 **현실성 · 경계 마감 · 미관** 세 관점으로 다섯 번 더 돌리며 굳은 규칙. ⑰의 11개 항에 이어진다.

12. **세로/가로 사각형을 잇대어 칠하지 않는다.** 조류대·눈 이랑·사구 능선·계곡 벽을 5~12px 조각으로 칠했더니
    전부 **빗살(바코드)** 또는 **스캔라인**으로 읽혔다. 띠는 위·아래 가장자리가 각각 굽이치는 **폴리곤 한 장**을
    그라데이션(또는 3단 계단)으로 칠한다.
13. **부드러운 그라데이션 띠는 "JPEG 뭉개짐·렌즈 먼지"다.** 픽셀 화면에서 소프트 방사 얼룩(그림자·눈더미·소·
    조수 웅덩이)은 오염으로 읽힌다. 윤곽이 있는 다각형 + 단계진 값으로 그린다.
14. **대체물 스프라이트는 축소할 때도 보간을 끈다**(`imageSmoothingEnabled = false`). 켜져 있으면 같은 소품이
    가까울 땐 픽셀, 멀 땐 매끈한 벡터가 돼 한 화면 안에서 화풍이 갈린다. 논리 해상도 상한은 72px.
15. **한 도형은 한 서브패스로.** path 중간의 `moveTo`는 좌·우 반쪽을 별개 서브패스로 만들고, 그 위의 `clip()`이
    한쪽만 잡아 **밝은 면이 도형 옆에 복제**된다(소나무 1차판의 "옆에 선 초록 유령").
16. **오프셋 폴리곤을 여러 겹 겹치지 않는다.** 폭이 변하는 리본(갯골·시내)에 1.0/1.24/1.55배 폴리곤을 겹치면
    굽이에서 자기교차해 본선과 어긋난 "각진 반투명 판"이 된다. 둔치는 **구간별 굵기의 스트로크**로 긋는다.
17. **먼 물체의 알파 하한은 0.78.** 0.55는 먼 나무 너머로 뒤 나무가 비쳐 앞뒤 관계가 사라진다(거리감은
    엔진의 안개 한 겹이 맡는다).
18. **작은 식물은 지평선 쪽에서 크기가 0.3까지 줄고, 무리는 원경에 더 자주 생긴다.** depthScale(0.6~1.0)만으로는
    지평선의 풀포기가 근경의 0.75배로 남아 "키 3~5m 짜리 풀"이 되고, 스프라이트를 거리로 솎으면 화면 상반부가
    빈 물감판이 된다(비스듬한 시점에서 먼 땅은 압축돼 밀도가 **올라가야** 한다).
19. **땅에 붙는 것은 리본의 단면까지 지평선 아래로 clamp.** 대각선으로 흐르는 리본은 법선의 세로 성분이 0.7을
    넘어 단면이 하늘로 20px 솟는다.
20. **자리 점유 필드(claimSpot)에 물길을 가장 먼저 등록한다.** 나중에 등록하면 그 전에 놓인 바위·관목이 물 위에 선다.
...
## ⑯ 개정 — 깊은 바다의 시간대(2026-09-06, 비주얼 QA 라운드 10)

옛 ⑯ "계절·날씨·시간대가 닿지 않는다"를 **"계절·날씨만 봉인, 시간대는 장면이 스스로 읽는다"**로 바꾼다. 소유자 우선순위 A(시간대 차이 강화)에서 깊은 바다를 별도 중점으로 지정했고, 검토 A·B·C가 동일 의견 — 장면이 수면 빛줄기를 늘 그리는 이상 "밤낮이 없다"는 "새벽 2시의 태양 기둥"이라는 모순이었다. 엔진 패스(입자·대기 안개·조명)는 그대로 건너뛰며(`sealed()`), `scenes/deep.ts`가 `f.time.band`만 읽어 빛줄기 세기·수·기울기 · 위 1/3 명도 · 밤 발광을 정한다. 검사 계약은 "30 해시 동일" → **"띠마다 5날씨 해시 동일(고유 해시 정확히 6)"**(`scripts/ambient-qa/selftest.mjs` ④·⑨). 규칙 정본: `docs/ambient/BIOME_GRAMMAR.md` §11.
