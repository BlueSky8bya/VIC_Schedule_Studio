...
### Version 3 — researched harmony correction, 2026-09-19

사용자가 v2의 과한 채도를 거절했다. v3는 기존 따뜻한 회갈색 배경에 맞춰 큰 채움의 OKLCH 채도 상한을 0.14에서 0.065로 낮추고 작은 강조색은 0.11로 분리한다. 원래 색조·무채색·라이트 원본은 보존한다. 연구/공식 디자인 지침과 적용 판단의 구분은 [팔레트 기록](../../ux/dark-mode-palette.md#v3-research-informed-harmony--2026-09-19)에 남긴다. 이 수식은 v2의 채도 증폭 결정을 대체한다.

`0123`은 기존 순수 변환 함수를 재사용하는 v3 생성 컬럼을 추가한다. 기존 v1/v2는 남겨 즉시 reader 롤백이 가능하다. 공개 DTO는 같은 허용된 시각 필드만 읽는다. 1,240색 SQL/TS 일치·43행 원본 보존·자동 생성 검증을 완료했다.

### Version 2 — owner correction, 2026-09-19

사용자가 실제 달력에서 색을 구분하기 어렵다고 지적했다. v1의 낮은 채도 상한과 다크에서도 남은 눈 편한 모드의 갈색 베일이 원인이었다. v2는 원래 색조를 유지하면서 파스텔 채도를 더 살리고 명도 폭·테두리 차이를 넓힌다. 다크 카드의 갈색 베일은 제거하고 약한 중립 광택만 남긴다. 글자 대비는 RGB 격자 검사에서 4.5:1 이상을 보장한다. 무채색은 색을 새로 부여하지 않는다.

`0122`는 v1 함수·컬럼을 바꾸지 않고 `dark_palette_v2` 생성 컬럼을 추가한다. 공개/편집실 reader는 그 컬럼을 같은 명시적 시각 DTO로 읽고 낡은 version 1 메타데이터는 즉시 다시 계산한다. 신규·색 수정 자동 저장 계약과 원본 라이트 색은 유지한다. 롤백은 v1 reader로 가능하다.
