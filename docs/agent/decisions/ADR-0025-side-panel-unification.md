...
The owner asked for UI unification on 2026-09-17 and answered three explicit choices: the viewer preview panel stands **outside the poster surface in one column, like the studio** (the in-surface right rail state "끔" is retired); a panel that auto-collapsed on a narrow window **floats over the calendar** when reopened instead of pushing it; the auto-collapse threshold applies to the studio, the viewer preview and the anonymous public viewer alike. The `/onair` fixed scene is not a user-operated surface and keeps its always-open composition.

Fourth pass: panel side/collapse sync between hook instances via a same-window custom event (`vic:side-panel`); calendar motion on panel changes goes through **one helper**, `lib/ui/flip-motion.ts` (`flipSpring`) — studio scales on collapse/expand (origin = corner opposite the panel), viewer translates on side switch — same curve and length as the panel slide; window resizes never animate. The two calendars remain structurally different (fixed 1840px canvas vs fluid grid), so the shared pieces are the hook, the pill, the token and the FLIP helper.

Third pass (owner review of the second deployment, same day): threshold is **1680px**; the viewer panel is an in-flow **sticky column** inside `.poster-fit` (flex row, order by side) instead of a fixed independent scroller — wheel over the panel scrolls the page, a panel taller than the window gets a negative `--slot-top` so its bottom sticks, collapse animates the column width to 0; the studio rail divides its width/edge by `--shell-zoom` so the on-screen card width equals the viewer's (token − 20px, 10px edge); the pill's zoom cell is information only (1.2s, no proximity wake) and hovering the pill always shows the panel toggle; account/preview cards are measured from layout values (`offsetWidth` + `.poster-fit` rect) so a side switch never bounces them.

Same-day revision (owner review of the first deployment): threshold raised from 1280px to 1540px (`PANEL_WIDE_MIN`, later 1680); side buttons ⇤ ⇥ are shown to anonymous viewers too; the panel width is one token `--side-panel-w` (`clamp(300px, 18.75vw, 380px)`) for studio and viewer; the studio scales the whole calendar by `panel.fit` (= 1 − panel/viewport width) while the panel pushes, so a month stays in one view like the poster's width-fit; the zoom badge (`125% 초기화`, studio −/%/+) is folded into the pill's middle cell while awake; the pill's middle toggle shows state by ink only, side buttons by the white box; the viewer account card and the preview-navigation card align to the panel card when the panel pushes on the right, otherwise to the calendar grid's right edge; the studio account card side-shift applies only while the panel actually pushes; the D+ card sub-line is the debut date, and mobile shows only `데뷔 N일째`.
