...
큰 채움과 작은 강조색의 채도를 분리해 기존 회갈색 배경과 조화시킨다. `0123`은 원본·v1/v2 컬럼을 보존하는 v3 생성 컬럼만 추가한다. SQL/TS 1,240색 일치·43행 원본 보존 확인. 롤백은 v2 reader 배포. [근거·검증](../ux/dark-mode-palette.md), [ADR-0026](decisions/ADR-0026-automatic-dark-tag-palette.md).
...
### CHG-20260911-002 — 봄 초원 3레이어와 넓은 하늘 (로컬)

새 원경·지면·근경 아트와 봄 전용 .35 지평선으로 넓은 하늘·작은 풀·빈 중앙을 적용했다. 렌더·입력·날씨 좌표를 같은 범위로 묶고 다른 바이옴의 .26은 유지한다. RGB 원경/근경의 바탕은 원본 수정 없이 측정 마스크로 제외한다. 복구는 새 아트 연결과 좌표 범위를 함께 ADR-0023 상태로 되돌린다. 원본·이전 공개 파일은 보존했다. [ADR-0024](decisions/ADR-0024-spring-meadow-three-layers.md), [검증/한계](../ambient/rounds/ROUND-21-meadow-camera.md). 추가 푸시·배포 없음.

### CHG-20260911-001 — 감상 모드 깊이와 봄 초원 PC 배경

사용자가 선택한 열린 구도와 세밀한 픽셀 느낌을 봄 초원에 적용했다. 감상 모드에서만 깊이·장식 움직임을 실행하고 자동조절은 관측된 프레임 부하에 따라 효과를 줄인다. 모바일 최초 진입은 장면·새 배경을 불러오지 않는다. 원본 해상도는 1536×1024로 유지하고 원경 캐시를 제한했다. 심해 계절 봉인, KST 월 기준, 공개 데이터·권한 경계는 유지한다.

복구: 이 변경 커밋을 되돌리면 이전 절차적 배경으로 돌아간다. 봄 아트만 되돌릴 때는 ADR-0023의 연결 해제를 따른다. 원본·고정 입력·생성 요청은 초원 작업회차에 보존했다. 검증: typecheck, lint, 803 tests, build, 최종 봄 화면 35검사·깊이 9검사·3개 결정성 smoke 통과. 독립 A/B/C 검토, 성능 비교와 한계는 [ROUND-20](../ambient/rounds/ROUND-20-spring-release.md), 선행 엔진 검증은 [ROUND-18](../ambient/rounds/ROUND-18-depth-P0.md). 푸시·배포 확인은 ROUND-20에 별도 기록한다.
