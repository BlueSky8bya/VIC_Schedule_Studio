# Historical snapshot: CLAUDE.md

Status: Archived (2026-09-09)

Historical evidence only. This snapshot is not current instructions and must not be loaded by startup hooks or task routing. Original relative paths in the fenced source refer to the original file location. Consult current topic rules and task state.
...
- **Ambient / biome background work (`components/shared/ambient/**`) → `docs/ambient/README.md` first** (2026-09-05).
  That folder holds the standing rules and the QA loop; do not re-derive them from the long bullets above:
