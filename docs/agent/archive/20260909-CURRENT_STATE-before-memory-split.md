# Historical snapshot: docs/agent/CURRENT_STATE.md
...
Source UTF-8 bytes: 289226
Source SHA-256: 6bc050b84e390cae512a93630f88b340e96af1c484e55e6fa35ad2c6288fe345
...
- **▶ 다음 세션 진입점 — 소나무 21장 4차 의뢰(2026-09-09). 할 일은 "보내기" 하나다.**
  **인계 문서: [docs/ambient/HANDOFF-20260908-pine.md](../ambient/HANDOFF-20260908-pine.md)** — 3차 반려 실측·다음 할 일·프롬프트 다시 뽑는 법이 거기 다 있다.
  보낼 것: [docs/ambient/prompts/20260909-pine21-r4.md](../ambient/prompts/20260909-pine21-r4.md) **+ 첨부
  `docs/ambient/reference/tree-pine.png`**(첨부가 빠지면 3차와 같은 빈칸이 다시 생긴다). 계절 셋은 **한 배치**로.
  `public/ambient/art/`의 소나무는 아직 **합격본 `-1` 세 장뿐**이고, 반려본은 2차 `art-src/incoming-pine/` ·
  3차 `art-src/incoming-pine-r3/`에 커밋돼 있다.
  **3차 반려의 교훈 한 줄: 말은 새고 그림은 안 샌다.** 3차는 **수치 여덟을 전부 통과했다**(도트 결·고아·얼룩·외곽선·
  다양성 최대 77.7%·계절 짝 100%·눈의 양 22.9~31.1%·노랑기 0.0%). 틀린 둘은 **말로만 적혀 있던 것**이다 —
  눈을 단 윗면에 얹지 않고 **윤곽에 흰 테**를 둘렀고(눈이 테두리에 몰린 비율 합격 20.8% 대 납품 38~48%),
  밑동이 **게 다리처럼 세 갈래**다(밑줄 가닥 수 합격 1 대 납품 3, 21/21). 뿌리는 브리프의 "합격본 파일을 열어 보고
  맞춰라" — **생성기는 그 파일을 볼 수 없다.** 그래서 레퍼런스가 파일 이름에서 **첨부 그림**이 됐다
  (`scripts/ambient-art-reference.mjs`, 확대는 정수배만 — 비정수배는 "흐리게 그려라"를 가르친다).
  새 결함 둘도 지표가 됐지만(`npm run art:check`) **수치 통과는 합격이 아니다** — 마지막 판정은 시트 옆에 놓고 눈으로 한다.
  ⚠ 코덱스가 합격본 `-1`을 덮어쓴 적이 있다 — 보관 후 되돌렸다. **덮어쓰기를 보면 먼저 보관하고 되돌린다**(2026-09-07에 반대 사고가 있었다).

- **✅ 바위 8장 합격(2026-09-08)** — 1차 반려 사유 4건이 실측으로 전부 닫혔다: 최암 톤 **33~42% → 8~17%**(기준 20) ·
  중간 톤 L **55 → 114~192**(눈밭·모래에서 구멍으로 안 보인다) · 팔레트 **4장 동일 → 8/8 고유** · 삐져나온 꼬리 0.
  게이트도 통과(색 ≤6 · 반투명 0 · 가로 연속 최빈 64 · 채움 69~81%). 겨울 산·암석해안이 1차와 비교가 안 된다.
  남은 흠 둘은 **내 브리프 잘못**이었다 — 자리 비 1.3:1에 "세로로 선 돌"(비 0.6)을 요구해 그릴 수 없는 걸 시켰고,
  그래서 여덟 비가 1.17~1.56에 몰렸다. 브리프를 "차이는 폭이 아니라 **윗선**에서 낸다"로 고쳤다.

- **✅ 프롬프트 생성기 수술 3건(2026-09-08)**
  ① **브리프에 "변형 N개"를 쓰지 않는다** — 변형 수를 올린 뒤 **16곳이 옛 개수를 코덱스에 부탁하고 있었다**(소나무 2, 낮은구름 4, 풀 3…).
     22곳에서 걷어냈고 계약 테스트가 재등장을 막는다. 개수는 표의 파일 목록과 '한 화면' 칸이 말한다.
  ② **소나무 짝 규칙을 일반화** — `-1`·`-2`만 이름으로 가리켜서 8변형에선 `-3`끼리 같은 나무라는 말이 없었다.
     장면이 달에 따라 파일을 갈아 끼우므로 짝이 깨지면 **달력을 넘길 때 나무가 바뀐다**.
  ③ **긴 브리프가 마크다운 표를 깨던 것** — 표는 한 줄 요약(색인), 전문은 `## 자리별 상세`로. 계약 테스트가 표 행이
     한 줄이고 열 수가 머리와 같은지 검사한다.

- **✅ 자연의 다양성 + 아트 보드 자리 상세(2026-09-08, PLAN-009)** — 소유자: "돌 종류가 최소 8개는 되어야지. 나무나 구름도. 자연이 얼마나 다양한데."
  실측이 그 말을 뒷받침했다 — 한 화면 동시 개수 대비 변형이 절반도 안 됐다(소나무 12~20그루에 2변형 = **6~10회 반복**, 낮은 구름 30덩이에 4변형,
  바위 12개에 4변형). 검사관 A가 이미 "같은 실루엣 6회 = S-3 몰입 파괴"로 판정한 기준을 세 자리가 위반 중이었다.
  ① 자리에 `perScreen`(장면 코드에서 센 값)을 적고 `recommendedVariants` = ceil(perScreen/2)(정체성 상한 8·잔 소품 6, 화면 개수는 안 넘음)가 권장 수를 낸다.
     **계약 테스트가 강제**하므로 새 자리가 조용히 모자랄 수 없다. 30자리 상향 — 바위·낮은구름·소나무 3종 8, 참나무·중간구름·갈대·연잎 6, 관목 5.
  ② **프롬프트가 이유를 말한다** — "이 그림은 한 화면에 최대 12개가 동시에 놓인다. 그래서 변형이 8개다." 1차 바위가 팔레트 비율까지 똑같이 온 건
     "복제 금지"라고만 하고 복제의 대가를 안 알려 줬기 때문이다.
  ③ **아트 보드에 자리 상세**(`/studio/ambient-art/[slotId]`) — 변형이 칸으로 보여 빈 다섯 장이 산수가 아니라 빈 액자로 읽히고, 규격(크기·비·격자·블록·
     가로 도트·저장 배수)이 한 블록에, "왜 이만큼인지"가 그 옆에. 프롬프트 복사는 자리 전체 / 남은 것만. 목록의 카드 제목이 진입점이다.
     화면을 보고 잡은 것 셋: 브리프의 마크다운이 별표로 찍히던 것, `previewOf`의 캔버스 하나를 여덟 칸이 뺏어 마지막 칸만 보이던 것, 바위 브리프가
     변형 4개 기준으로 남아 있던 것.

- **✅ 앰비언트 QA 라운드 17 완료 — 소유자 육안 3건 + 비의 젖음(2026-09-08)** — 상세 [docs/ambient/rounds/ROUND-17.md](../ambient/rounds/ROUND-17.md).
  검사 에이전트 A·B·C **동시 검토**로 복귀(P0 3 · P1 16 · P2 16)했고, 소유자가 같은 날 준 3건이 전부 그 발견과 같은 뿌리로 수렴해 묶음에 흡수됐다.
  ① **민물의 경계를 곡선 하나로**(P0 2) — 위쪽만 `waterTopAt(x)` 곡선이고 아래는 상수 `waterBottom()`이라 오리가 기슭 안쪽 107px까지 들어간 뒤
     **완전 반사**(`vy = ±|vy| + 4`)로 튀어나왔다(소유자: "던지면 들어갔다가 튕겨져 나온다"). `nearShoreYAt(x)` 신설 — **그리는 선과 판정하는 선이
     같은 식**이고(굽기 쪽이 그 함수를 되돌려 쓴다), 경계는 반사 → **흡수**. 연잎은 전역 지면 매핑이라 60%가 기슭 판에 덮여 12장이 4~5장으로
     보였다 → 물 폴리곤 안으로 사상. `midWater` 한 장에 굽던 바위 3 + 통나무 1을 **개체 + 발 y**로 떼어 오리와 한 대열로(소유자: "돌이나 통나무
     위로 헤엄친다" — 라운드 16이 초원 가을에서 닫은 AMB-A3-02와 같은 구조 결함). 실측 4계절 × 2시드 × 20초 = **640프레임 물 밖 0**.
  ② **하늘이 원근을 배운다** — 폭 ≥ 60px 덩이의 cy 평균 118 vs 작은 것 68으로 **아래로 갈수록 커졌다**(깊이 단서 "크기"의 원근 역전).
     티어 표를 뒤집고, 구름 하한을 지평선이 아니라 **원경 상단**(`bakeClouds(..., floorY)`, 기본 `hillCrestY`; 바다·해안은 수평선)으로:
     최하단 **178 → 142**(마루 153, 소유자 "구름이 언덕 아래까지"). 혜성은 진행각이 정확히 180.0°라 UI 티커로 읽혔다 → 하늘 띠가 좁아
     **가로 폭을 줄여 기울기를 사서** 174.3° · 67 → **27px/s**(26초 지속 유지 — 혜성을 혜성이게 하는 건 크기가 아니라 그 시간이다).
     새털은 칸 배정 + 플립 + ±10°로 전폭 리본 해체(cy 표준편차 하늘 띠의 25.9~29.2%).
  ③ **비의 젖음 — 라운드 16의 진단이 틀렸다** — 하늘 베일이 만드는 far-band는 `groundYAt(.30)`, 안개는 `hazeEndY`에서 끝나 **화면 아래
     절반에 물리적으로 닿지 않는다**(맑음에 mul·앵커만 적용한 예측이 실제 비 프레임과 소수 둘째 자리까지 일치). 범인은 채널 비대칭 곱셈
     `[.84,.86,.90]` — R을 B보다 6% 더 눌러 따뜻한 색(가을 갈색·마른 억새)의 R−B를 깎는다. `[.86,.86,.86]` + 앵커 .09 → .05,
     원경은 `skyAlpha` .38 → .28 · `hazeK` ×1.3 → ×1.15. 근경 크로마비(비÷맑음) **.62~.76 → .821~.851**(baseline .53~.68).
  ④ **바람 방향이 장면에 도달** — 라운드 14가 만든 `f.windDir`의 소비자가 `coast.ts` 하나뿐이라, 시드 42에서 빗줄기는 왼쪽으로 흐르는데
     `land.ts` 억새는 언제나 오른쪽이었고 `spring.ts`는 돌풍마다 방향을 새로 뽑았다. land·spring·autumn·winter가 소비하고,
     나무에 **평균 기울기 항**을 더했다(순수 사인이라 평균 변위가 0 = "흔들리지만 불지 않는다").
  게이트 tsc 0 · eslint 0 · vitest **643** · build 0 · `ambient:qa:selftest` ALL PASSED · **깊은 바다 봉인 불변**(s14 before→after 픽셀 동일).
  ⚠ **이 라운드가 스스로 만든 결함을 캡처를 눈으로 보고 잡았다.** 장면 클로저 최상단에서 `h`로 상수를 계산하면(그 시점 `h` = 0) 기슭선이
     화면 바닥으로 내려가 경계가 사실상 사라지고 근경 기슭 판이 아예 안 그려지는데, **게이트도 프로브도 같은 깨진 함수를 써서 전부 통과했다**.
     장면 최상단의 `w`·`h` 파생값은 **반드시 함수로** 둔다. 수치만으로는 못 잡는 종류가 있다.

- **▶ 아트 남은 분량 — 실측(2026-09-08, 매니페스트 vs `public/ambient/art/` 대조).**
  **1차 217장 · 자리 73개**가 남았고, 그중 **반복이 눈에 보이는 자리 30개**(`perScreen`이 있는 것)에 **140장**이 몰려 있다.
  배달 완료는 **43장** — `rock` 8 · `moon-phase` 8 · `sun-disc`·`comet`·`shooting-star` 각 2(이 다섯 자리는 완결) ·
  구름 4자리 14 · 참나무 4자리 각 1 · 소나무 3자리 각 1.
  ⚠ **"파일럿 잔여 11장"은 죽은 숫자다.** 그 목록(`rock-1~4` 등)은 **변형 상향(PLAN-009) 전**의 것이고, 바위는 그 뒤
  **8장 전부 받아 합격했다**. `pilot` 필드와 아트 보드의 **파일럿** 필터도 같은 이유로 낡았다 — 남은 장수는
  매니페스트가 정본이므로 **아트 보드의 「남은 파일만」**으로 뽑는다(자리가 아니라 파일 단위로 거른다).
  ⚠ **하늘도 아직 안 끝났다.** 26장을 받은 뒤 PLAN-009가 구름 변형을 8/6/4/4/4로 올려 **12장이 다시 남았다**
  (`cloud-low` 4 · `cloud-mid` 3 · `cloud-storm` 2 · `cloud-wisp` 2 · `cloud-high` 1). 완결은 천체 넷뿐이다.
  **순서**: 소나무 21(위 진입점) → 참나무 20 → 하늘 12 → 관목 20 → 잔 소품 → **도감 1차 43종(65장)** → 2차 → 3차.

- **✅ 하늘이 그림이 되었다 — 배달 · 연결 · 밤의 사건 · 개발자 손잡이(2026-09-08)**
  화면의 26%가 하늘인데 전부 절차적이라 늘 같았다. 이제 그리지 않는다 — 그림을 놓는다.
  ① **천체·구름 26장 배달**(자리 8개: 해 2 · **달 위상 8** · 혜성 2 · 별똥별 2 · 뭉게 4 · 중간 3 · 새털 3 · 비구름 2). **2라운드** 걸렸다 —
     1차는 비율이 어긋나(새털구름이 자리의 18%만 채움) 반려했다. 그래서 **자동 게이트 둘**을 `art:normalize`에 넣었다:
     **채움비 45% 미만 경고**와 **`floatingBar()`**(그림 안을 가로지르는 수평 막대 검출 — 참나무 줄기의 그 선을 잡은 검출기다).
     프롬프트·스타일 가이드에는 **자리 비율**(`ratioOf`)을 찍는다. 이유: "가로로 길게"라는 말만으로는 생성기가 정사각을 낸다.
  ② **엔진 연결**(`world/sky.ts`) — `skyArt = new ArtSet([...8슬롯])`, `skyKey`에 `a${skyArt.version}`(그림이 늦게 와도 다시 굽는다),
     `moonArt(phase)`가 음력 위상 → 8장 중 하나, 해는 띠로 고른다(노을 = 변형 2). 구름은 스탬프 수 `SC` **3 → 1**(한 장이 이미 구름
     한 덩이다)인데 개수 계수를 안 고쳐 **구름이 9배**로 깔렸다 — `areaK = (SC/3)²`로 정규화. 새털구름은 blob 경로 **앞에서 return**하던
     별도 분기라 따로 연결해야 했다.
  ③ **밤하늘의 드문 사건**(`world/sky-events.ts` 신설) — 별똥별·혜성. `SpawnDirector`를 쓰지 않았다: 하네스 계약이 "같은 URL = 같은 프레임",
     `advance(1000) == advance(250)×4`인데 상태를 쌓으면 **step을 어떻게 쪼갰는지**에 기대게 되고, 하늘 모듈 하나를 열한 바이옴이 공유해
     두 장면이 각자 step을 돌리면 사건이 두 배로 빨리 흐른다. 그래서 **일정표를 미리 정해 둔 것처럼** — 사건 i의 시각·성질이 (시드, i)의
     순수 함수. 평균 간격 별똥별 ≈ 64초 · 혜성 ≈ 8.8분(처음 잡은 18분은 대부분의 세션에서 한 번도 안 와서 **그린 그림을 아무도 못 봤다**).
     밤 + 맑음/바람에서만, `load` 아래면 접는다. 계약 10건(`tests/unit/ambient-sky-events.test.ts`).
  ④ **개발자 손잡이** — 설정 모달 개발자 줄 **하늘 사건**(자동/별똥별/혜성)과 **감상 톱니에 같은 줄**(같은 `world.force` 상태, 사본 아님).
     고르면 쉬지 않고 되풀이한다(주기마다 자리·방향·변형 재추첨) — 평균 9분을 기다려 확인할 수는 없다. 밤·맑음 조건은 **강제해도 안 건드린다**
     (실제 동작을 속이는 도구는 확인용으로도 나쁘다). fixture `?skyEvent=shooting-star|comet`.
  ⑤ **꼬리가 진행 방향을 따른다**(`a770b49`) — 배달본 넷 실측 결과 머리는 오른쪽, 축은 −16° 남짓인데 `rot = 0`으로 그리고 있었다.
     별똥별은 **+22.8°로 내려가면서 꼬리를 위로** 뻗었다(39° 어긋남). `aimSprite(travel, art, flip)`가 회전각을 낸다 — `drawArt`가
     **rotate → flip** 순서라 뒤집힌 스프라이트의 최종각이 `rot + (π − art)`이고, 부호를 틀려도 화면에선 "그럴듯하게 기울어" 보여 눈으로 못 잡는다.
     그래서 순수 함수 + 그 합성을 재현하는 테스트. 혜성 크기는 하늘 띠의 **24~34% → 8.5~11.5%**(소유자: "왜 이리 커") — 혜성이 혜성인 것은
     크기가 아니라 26초 걸려 건너간다는 것이다.
  ⑥ **참나무 줄기의 가로 막대 제거** — 소유자 지적. `floatingBar()` 검출기로 두 장에서 실측 확인 → 코덱스 재생성 → 적용.
     저장본이 블록 배수가 아니어서(348 vs 블록 8) 거짓 "옛 파이프라인" 경고가 나던 것도 함께 고쳤다(상자를 `block × n`으로 스냅).
  ⑦ **원본이 저장소로 들어왔다** — `art-src/`(30장 252KB, 서빙 안 됨). `.scratch-pw/`는 기기가 바뀌면 사라지는데, 원본이 없으면 자리 상자가
     바뀔 때 축소본에서 도트를 되살릴 수 없다. `art:normalize`가 덮어쓰기 전에 자동 보관한다.
     ⚠ 사고 기록: 2026-09-07에 배달본을 정규화한 뒤 `git checkout -- public/ambient/art/`를 돌려 **막 받은 그림을 옛 커밋본으로 덮었다**.
     소유자에게 재배달을 부탁해야 했다. `keepSource`가 그 재발 방지다.
  게이트 tsc 0 · eslint 0 · vitest **643** · build 0 · `ambient:qa:selftest` ALL PASSED.
  **지금 배달된 그림 33장** = 하늘 26 + 참나무 4(변형 1) + 소나무 3. 나머지 자리는 대체물(절차적·Noto)로 그려진다.

- **▶ 다음에 할 일 = 앰비언트 QA 라운드 17**(`docs/ambient/QA_PROGRESS.md` §6) — 첫 입구는 **비의 젖음(W-1 ③)**인데, 지면이 아니라 `world/light.ts`의 **rain 프로파일**부터다. 라운드 16이 지면 쪽 원시연산 세 가지를 전부 실측으로 떨어뜨리고 진범을 찾아 뒀다: 탈채도를 만드는 것은 지면이 아니라 비의 **전면 하늘 오버레이**(`skyAlpha` .38 회청) + `hazeK ×1.3`이라 지평선 아래까지 회색 베일이 덮는다 — 그 위에 무엇을 얹어도 다시 씻긴다. 되돌리기 전 baseline: 지면 채도 비÷맑음 **0.53~0.68**. 그다음 육지 정지 프레임 눕기(W-1 ②). 프로토콜 §7대로 검사 에이전트 A·B·C 동시 검토로 복귀한다. 아트(코덱스 소나무 파일럿)는 이와 **독립**이라 병행 가능.

- **✅ 도감 세 권 + 아트 보드 대개편(2026-09-07, PLAN-20260907-008 · ADR-0019)** — 소유자 지시: "동물의 숲처럼 도감 수집하는 맛. 동물도 우리가 새로 그린다. 식생·돌·구름·천체도 여러 버전이 있어야 한다."
  ① **`world/codex.ts` 신설 = 도감 정본 129종**(물고기 40 · 곤충 39 · 동물 50, **실재하는 한국 생물**). 종마다 **바이옴 · 달 · 시간대 · 날씨 · 비 온 뒤 · 희귀도 · 크기(cm) · 시점 · 변형 · 생성 차수 · 도감 한 줄**. 조건이 핵심이다 — 없으면 도감이 아니라 목록이고, "지금 여기 아니면 못 본다"가 달력을 넘기고 바이옴을 걸어 다닐 이유가 된다(피라미 = 사철·종일 = 언제 가도 만난다 / 장수풍뎅이 = 숲·7~8월·밤 / 달팽이 = 비 그친 뒤).
  ② **동물 규칙 뒤집힘(ADR-0019)** — "동물은 Noto 이모지만"(2026-09-04) **철회**. 화풍이 확정돼 이모지가 한 화면에서 혼자 다른 어법이 됐고, 무엇보다 도감의 주인공을 남의 그림으로 채울 수 없다. Noto는 **그림이 오기 전까지의 대체물**로만 남는다. `world/species.ts`(28종) 삭제 — 목록이 둘이면 "도감엔 있는데 그릴 자리가 없는 종"이 생긴다.
  ③ **아트 자리 69 → 206 · 파일 319장**: 2차 종 129자리를 도감에서 파생 · **천체·구름 8자리 신설**(해 2 · **달 위상 8** · 혜성 2 · 별똥별 2 · 뭉게/중간/새털/비구름 12 — 하늘이 화면의 26%인데 늘 같았다) · 정체성 초목에 변형 2(참나무·관목·자작나무 4계절씩, 그루터기·통나무·묘목). 참나무 배달본 4장은 `-1`로 리네임.
  ④ **보드**: 카드마다 도감 줄(사는 곳·달·때·희귀도·크기·차수) + 도감 한 줄, 머리에 세 권 진행률, 필터에 생성 차수와 하늘 갈래.
  ⑤ **계약 13건**(`tests/unit/ambient-codex.test.ts`) — 그중 둘이 눈으로 못 잡는 것을 잡는다: **그 달에 불가능한 날씨 조건 금지**(영원히 안 나오는 종)와 **바이옴마다 1차 종 1개 이상**(끝까지 비어 보이는 화면 금지).
  게이트 tsc 0 · eslint 0 · vitest **632** · build 0.
  **다음 생성 순서**(⚠ 낡음 — 현재 순서는 맨 위 **아트 남은 분량** 항목이다): 파일럿 잔여 11장 → **천체·구름 26장**(달 위상이 날짜의 표정이 된다) → 참나무·관목 변형 8장 → 도감 1차 43종 → 2차 → 3차. 얇은 바이옴(산 10 · 암석해안 9 · 깊은 바다 7 · 먼바다 1차 1종)은 2차 배치에서 먼저 채운다.

- **(역사 · 2026-09-07 · ⚠ 숫자는 낡았다) 아트: 파일럿 잔여 11장** — 아래 목록·순서는 **변형 상향(PLAN-009) 전**의 것이다. 바위는 그 뒤 8장 전부 합격했고, 남은 장수는 매니페스트가 정본이다(맨 위 항목 참조). 남는 것은 방법 하나뿐 — `/studio/ambient-art`의 **「남은 파일만」** 버튼이 프롬프트를 만든다(자리가 아니라 **파일** 단위로 걸러 이미 합격한 그림을 다시 부탁하지 않는다). 옛 목록: `tree-pine-2` · `tree-pine-autumn-2` · `tree-pine-winter-2` · `shrub-autumn` · `reed-1,2` · `rock-1~4` · `sea-stack-1`. 옛 권고 순서 = **바위 4장 먼저**(사철 전 바이옴에 깔리고, 격자 16칸/블록 64px로 파일럿 중 도트가 가장 굵어 규격 위반이 제일 잘 보이며, 변형 4개라 '복제 금지' 규칙이 처음 검증된다) → 통과 후 나머지 7장.
  **소유자 결정(§10-4 ⓐ)**: 소나무는 **계절마다 2형**. 봄·여름만 2형이면 반쪽이다 — 장면이 계절마다 다른 파일을 고르므로 가을·겨울 산이 한 장을 40여 번 찍는다. 파일럿 12 → **14장**. 변형을 더하면 파일명이 `<id>-1.png`로 바뀌므로 배달본 둘을 리네임했다(`tree-pine-autumn.png` → `-1`, 겨울도).
  **바로잡은 것 둘**: ① 프롬프트의 스타일 가이드가 아직 "옛 참나무는 기준이 아니다 · 재작업 대상"이라고 코덱스에 보내고 있었다 — **재작업본이 이미 들어와 있다**(실측 참나무 색 8개·가로 연속 9.4px, 소나무 7~9개·9.5~9.9px). 기준선을 **일곱 장 전부**로 고쳤다. ② `rock`·`reed`는 변형 4개를 요청하면서 브리프엔 "변형 2개"라고 적혀 있었다(표와 설명이 어긋남) → 4개로 맞추고 무엇이 달라야 하는지를 적었다. ③ `.scratch-pw/dot-check.mjs`가 파일명·치수를 상수로 박아 **무엇을 넣어도 참나무(여름)를 쟀다** — 인자를 읽도록 고쳤다.

- **✅ 앰비언트 QA 라운드 16 — 초원의 레이어와 바람(2026-09-07, `docs/ambient/rounds/ROUND-16.md`)** — 소유자가 가을 화면에서 지목한 2건 + 라운드 15가 넘긴 W-1을 한 라운드에. 검사 에이전트는 띄우지 않았다(입구가 이미 확정돼 예산 4묶음이 찼다).
  ① **AMB-D3-04 — 안개가 화면 높이로 걸렸다**: `autumn.ts`의 `mist` 층(화면 위 34%, 라운드 3에서 배율만 걷어낸 잔재)을 **삭제**하고, 엔진에 `Scene.splitHaze()`·`Scene.drawAbove()`를 신설해 그 장면만 `draw → 안개 → drawAbove → 입자` 순으로 돌린다. 서 있는 것은 `view.ts hazeAt(발치 y)` 값 하나를 `source-atop`으로 **몸 전체에 균일하게** 섞는다(알파 감쇠가 아니라 색 혼합 — 라운드 12의 유령 결함 회피). ⚠ 배선: `world-scene`이 열한 바이옴을 감싸므로 **메서드 존재로 판정하면 안 된다**(첫 시도에서 래퍼가 위임을 안 해 `drawAbove`가 아예 안 불렸고 초원의 1초 변화가 0.00%가 됐다 — `fogFloor`가 라운드 11에서 당한 것과 같은 결함). 실측: 지평선 아래 띠① 국소 대비 **0.81 → 2.49**(숲 대조군 2.01 상회), "특정 높이 위가 밝다"는 계단 **+4.2L → −2.1L**(숲 1.7 · 초원 겨울 1.8).
  ② **AMB-A3-02 — 낙엽이 모든 나무 앞**: 나무를 `bakeGround`에서 빼 자리만 기록하고(결정적 rng 소비 순서 보존) `drawAbove`에서 낙엽·다람쥐와 **y 오름차순 한 대열**로 그린다(`land.ts`가 숲에서 쓰던 방식). 잎의 "전체 그림자 → 전체 몸통" 2패스도 한 잎씩으로 합쳤다.
  ③ **W-1 ① 초원 네 장면이 바람을 안다**: `currentLight().wind` 소비자가 **0개**였다(`land.ts` 5 · `coast.ts` 13). 가을 = 나무 삼단 흔들림(바람에 **초선형** ^1.3 — 선형이면 맑음 기준선이 같이 올라 구별이 안 된다) + 돌풍 간격을 `weather === "wind"` 계단에서 연속으로 · 봄/여름 = 풀포기 2차원 진행파(상시 흔들림은 절반으로 내려 그 몫을 바람으로) · 겨울 = 눈송이·눈가루가 바람에 비껴 내린다 · 민물 = `drawWaves`를 wind > .12에서만(맑음·안개는 잔잔한 못 그대로). 실측 바람÷맑음 지면 변화: 봄 0.88 → **2.43** · 여름 0.98 → **2.30** · 가을 1.06 → **2.10** · 민물 1.38 → **3.92**(육지 대조군 1.93~3.99).
  ④ **되돌림 — W-1 ③ 젖음 채널**: `world/surface.ts`·`Light.wet`·`Scene.wetK`를 만들었다가 전부 철거했다. 게이트는 통과했지만 언덕 비 화면이 청록 지면 + 주황 억새 + 새파란 바위로 물들었다. 원시연산 3종 실측 탈락 기록은 ROUND-16 §되돌린 것에 남겼다(`saturation` 블렌드 = 색을 **만든다**, 회색 뻘 C* 3.69 → 18.48 · `overlay` = 채널 차를 눌러 채도가 **떨어진다** 0.39~0.59 · 자기 자신 `multiply`(감마) = 물리적으로는 맞지만 점심 비가 −18L로 밤처럼 어둡고 부호가 안 뒤집힌다).
  게이트 tsc 0 · eslint 0 · vitest **617** · build 0 · selftest ALL PASSED. **귀속**: 대상(s03·s04·s16) 18/18 변경, 민물 3프레임 0.00%, **손대지 않은 12 시나리오 216 프레임 전부 동일**(깊은 바다 봉인 포함). 계획서 [PLAN-20260907-007](plans/PLAN-20260907-007-round16-layers-wind.md).

- **✅ 참나무·소나무 아트가 화면에 섰다 + 초원 리베이크 버그 수정(2026-09-07)** — 코덱스가 소나무 3장(변형 1·가을·겨울)과 **참나무 4장 재작업본**을 납품했고, `npm run art:normalize`가 1024 → 512(÷2 정수배, nearest)로 줄여 저장했다. 실측: 파일마다 **색 6~8개 · 반투명 0 · 가로 연속 최빈 = 자리의 블록**(참나무 8px, 소나무 16px) · 수관 면적 봄 75.6k < 가을 93.7k < **여름 100.9k**(계절 크기 .85/.95/1.0 계약). 남은 것: `tree-pine-2.png`(변형 2, 반복감 처방)와 나머지 빈 자리.
  **함께 고친 버그**: 자리 PNG는 비동기로 오는데 "아트 버전이 바뀌면 바탕 다시 굽기" 확인이 `resize()`에만 있었다 → 리사이즈가 없는 화면은 첫 바탕을 세션 내내 유지했고, **초원(가을) 나무 12그루가 전부 코드 대체물**이었다(소유자가 "소나무 하나가 옛 버전"으로 발견; 계측 `tree-oak-autumn:fallback 11`). `spring/summer/autumn/winter`도 `step()`에서 같은 확인을 하도록 — `land.ts`·`coast.ts`는 원래 하고 있어서 숲만 멀쩡했다.

- **✅ 시간대가 실제 태양 궤도를 따라 연속으로 흐른다 + 날씨는 하루 세 마디(2026-09-07, PLAN-20260907-006)** — 소유자 지시. ① `world/sun.ts` 신설(NOAA 근사, 서울·KST): 황도 경사 → 적위·균시차 → 일출·일몰·남중·남중고도·박명. 실측 서울 일출/일몰 **±3분**(하지 05:11/19:56 · 동지 07:43/17:17), 남중고도 동지 **29°** / 하지 **76°**. ② `world/time.ts`: 여섯 띠가 **칸에서 거점으로**. 위상의 축은 시각이 아니라 **태양 고도**(낮은 남중고도로, 박명은 −18°로 정규화) — 그래서 계절·날짜마다 저절로 달라진다. `time.band`(가까운 거점 이름)의 의미는 그대로라 스폰 풀·도감·QA·개발자 강제가 안 깨졌고, `from/to/mix`가 새로 나온다. ③ `light.ts lightAt()`이 두 거점을 `lerpLight`로 섞고, **그림자 길이에 실제 고도**를 얹는다(1/tan, 0.85~1.7배 — 겨울 정오 그림자가 여름보다 길다). 엔진의 3초 전이는 **불연속 변화**(날씨·거점 구간)에만 건다(아니면 `lightStable`이 영영 false → 그림자 재굽기 정지). ④ `sky.ts sunYOf`: 해 원반 높이를 고도에서. ⑤ `world/weather.ts`: 오전·오후 두 마디 → **세 마디**, 경계는 날짜 시드 랜덤이되 **각 마디 ≥ 4시간**(`SEGMENT_MIN_H`), 55%는 앞 마디를 잇는다. `weatherAt` → `{now, prev, segment: 0|1|2, until}`. ⑥ fixture에 `hour=` 파라미터(띠 대신 연속 시각으로 세운다).
  **눈으로 확인**: 같은 19시가 여름엔 노을(고도 +9.1°, noon→dusk 96%), 겨울엔 한밤(고도 −19.7°, 달 뜸). 게이트 tsc 0 · eslint 0 · vitest **617**(태양·연속 위상 7건 신설) · build 0 · `ambient:qa:selftest` ALL PASSED.

- **✅ (닫힘 2026-09-07) 코덱스에게 소나무 3장을 뽑아 화풍 판정 — 결정 1b, 소유자 승인 "much better"** — 막혀 있던 소유자 결정이 **ⓐ′**로 정해졌다(보간 끄기 + 정수배 축소 + 자리별 도트 격자). 파이프라인·프롬프트·보드가 그에 맞게 고쳐졌으니 **이제 생성만 남았다.**
  **들어간 것**: ① `art/load.ts` — `drawArt`·`bake`에 `imageSmoothingEnabled = false`(`bake`가 도트가 실제로 죽던 곳: 저장본 → 자리 상자 축소) ② `manifest.targetEdge` — 저장 변을 **1024의 정수 약수**(128·256·512)로만(옛 식은 192·320 같은 비약수를 냈다) ③ `manifest.dotGrid(px)` — **자리마다** 도트 격자(16·32·64·128칸, ≈ 긴 변 ÷ 2). 고정 128은 틀렸다(조약돌 도트가 0.4 장치px). 화면 도트 1.5~8 장치px를 테스트로 못 박음 ④ `scripts/ambient-art-normalize.mjs` — `nearest`·디더 0·블록 배수 되붙임·2의 거듭제곱 축소·`--force` ⑤ 프롬프트가 자리마다 **격자·블록**을 싣는다(`batchPrompt` 신설 — 아무 자리 묶음이나 한 장으로) ⑥ 매니페스트에 `pilot` 필드 — 파일럿 12장(소나무 2형+가을+겨울·바위 4·시스택 1·관목 1·갈대 2)이 자리 표에 박혔고 `tree-pine`은 **변형 2**가 됐다(반복감 처방) ⑦ 낡아 있던 `now` 14개 교정(`"none"`인데 코드 대체물이 있던 것들) → **빈 자리 = 정확히 26**.
  **보드(`/studio/ambient-art`)도 함께**: 빈 자리를 대기와 갈라 따로 세고(붉은 띠) 거른다 · 파일럿 배지·필터·프롬프트 · 자리마다 격자/블록/저장 변(÷비율) · 옛 규격(lanczos3) 파일 경고 · 지금 화면 필터(대체물 있음/빈 자리)·정렬 3종·`/` 검색 · 카드 98장을 화면에 들어올 때만 굽는다(IntersectionObserver — `display:contents`엔 옵서버가 안 걸린다) + 메모이즈 + 검색 지연.
  **주의**: 이미 저장된 참나무 4장은 lanczos3 시절 산물이라 다시 정규화해도 도트가 안 돌아온다(정보가 없다). 되살리려면 1024 원본이 필요하다 — 보드가 그런 파일에 경고를 띄운다.
  게이트: tsc 0 · eslint 0 · vitest **609**(아트 계약 4건 신설) · build 0 · `ambient:qa:selftest` ALL PASSED.
  **다음 순서**: 보드에서 **파일럿 프롬프트** 복사 → 코덱스에 소나무 3장 먼저 → **화풍만 승인**(§10 결정 5 ⓑ) → 나머지 9장 → `npm run art:normalize` → 게이트 → **라운드 16-A(아트) QA**(before = `.scratch-pw/qa/r15/after/` 재사용).

- **~~▶ 다음에 할 일~~ = 엔티티 이미지 에셋 파일럿(2026-09-06 판단 완료, `docs/ambient/ENTITY_ART_PLAN.md`)** — 라운드 15로 자율 QA가 끝났고, 소유자가 "이제 실제 이미지 기반 Entity Asset을 시작할 시점인가"를 물어 판단서를 냈다. 결론 **CONDITIONAL GO**. **이미지 생성은 코덱스가 한다**(소유자 확정) — 나는 프롬프트·엔진·QA만.
  **막힌 지점: 소유자 결정 하나를 기다리는 중.** `drawArt`의 보간을 끌 것인가(§10 결정 1b, C4). 실측: 저장 파일(460×512, 색 76 · 평균 가로 연속 2.75px)을 자리 상자 120×150으로 줄이면 **lanczos3에서 색 3,723 · 연속 1.10px · 1px 비율 95%** = 화면에서 픽셀 구조가 사라진다(nearest는 색 76 · 1.60px 유지). 이대로 파일럿을 받으면 **흐려진 결과를 보고 코덱스를 탓하게 된다.** 뭉개는 곳 둘: ① `scripts/ambient-art-normalize.mjs`의 `kernel: "lanczos3"` ② `art/load.ts drawArt`의 기본 보간(라운드 13에서 `imageSmoothingEnabled = false`를 넣었다가 "rock 아트가 없어 무효"라며 되돌린 그 줄 — 아트가 오면 되살려야 한다). 권고 = ⓐ 보간 끄기(`depthScale`이 .05 단위 양자화라 흔들림 완화). 도구 `.scratch-pw/dot-check.mjs`.
  **결정되면 곧바로 할 일**: 소나무 3계절 프롬프트(`slotPrompt` 또는 `/studio/ambient-art` 개발자 라우트)를 뽑아 코덱스에 넘기고 → 3장 받아 **화풍만 먼저 승인**(§10 결정 5 권고) → 통과하면 나머지 9장(바위 4변형·시스택·관목·갈대 2) → `npm run art:normalize` → 게이트 → **라운드 16-A(아트) QA**(before는 `.scratch-pw/qa/r15/after/`를 그대로 쓴다, 재캡처 불필요; 검사관 3종 동시, 자리 단위 되돌림).
  **결정된 것**: 눈 쌓임은 **코덱스가 겨울 변형 PNG에 그린다**(겨울 참나무가 그 증거, 눈은 실루엣을 바꾼다). 동적 누적은 보류(검사관 C가 "예산 밖" 2순위로 올림). 엔진은 **쌓이면 안 되는 자리**만 맡는다(조간대·물가 40px·수면).
  **핵심 수치**: 아트 자리 **69개 선언 · 4개 배달 · 33개 코드 대체물 · 6개 이모지/svg · 26개는 아무것도 안 그려짐**(시스택·큰 바위·억새·부들·통보리사초 등 = 검사관들이 지적한 정체성 결손과 일치). 소나무는 **변형 1개**로 산에 40여 그루가 선다(반복감의 주범). 전략 = **C+**(서 있고·16px 이상·5개 이상 반복·정체성을 지는 것만 이미지, 나머지 절차적 유지). **동물은 만들지 않는다**(Noto 규칙 유지 — 소유자 결정 대기 항목 §10-2).


- **앰비언트 비주얼 QA 라운드 15 완료 = 자율 6라운드 종료(2026-09-06, `docs/ambient/rounds/ROUND-15.md` · 총평 `docs/ambient/FINAL_AUDIT.md`)** — 마무리 묶음. 세 검사관이 각자 다른 첫째를 지목했고(A "하늘만 상태를 먹는다" · B "지형에 그 모양을 만든 힘이 없다" · C "면이 상태를 갖지 않는다") 교집합이 수평선 띠. ① **지면 앵커**: `Light.ground = {rgb, alpha}` 신설 — 지평선 **아래에만** source-over 한 겹(8단 세로 램프, 절단선 방지). `mul`은 순수 곱셈이라 밝은 지면이 목표까지 안 내려가 점심→밤 하늘 −30~−39L인데 지면은 −8~−13L이었다(추종률 .27~.41 · 11/11, 민물은 물이 밤하늘보다 밝았다 = 부호 역전). source-over는 `new = old·(1−a) + c·a`라 밝은 것을 더 많이 끌어내린다. 띠별 α 새벽 .05·노을 .03·저녁 .10·**밤 .15**(겨울 눈 알베도 ×.62), 날씨 흐림 +.05·비 +.09·눈 +.02. 실측 추종률 **.47~.63(10/11)** · 밤 하늘−지면 6.7~23.6 · 비 지면÷하늘 .33 → .47 · 안개 D-map 단조 5/7 → **6/7** ② **수평선 반사 띠를 굽지 않는다**(6띠 × 4날씨 24칸 중 18칸이 규칙 밖): `bakeWater`의 흰 자(`softBlob`) 삭제 → `drawHorizonGlow`가 매 프레임 `glint × (1 − skyAlpha)`로 40~160px **마디**를 띄워 그리고, 안개면 띠를 끄고 물 상단을 `hazeRgb`로 눕혀 수평선을 지우며, **물 상단 60px를 하늘 덮은 만큼 덮는 베일**(색은 `hazeRgb` — `L.sky`는 색조만 옮기는 밝은 판이라 베일로 못 쓴다). 비 17.6 → **9.0** · 안개 ≈ 0 · 맑음 저녁 8.1 → −0.6 ③ **직전 두 라운드 결함 복구 3**: 거품 마디가 바람에 굵어져 186×16px 흰 테이프(라운드 14) → 짧게·촘촘하게 + y ±2px 흩기 · 비 고리 원근 0.98(라운드 14 수용 기준 ≤.6 미달) → 스폰 `u`로 ×(0.45+0.75u) · 물가 바위 젖음 채널 부재(라운드 13 층, 갓 드러난 돌이 화면 최고 +8.5L) → 물가 40px 안 `source-atop` 젖음 + 조류대 스커트. 회귀 전부 유지 · selftest ALL PASSED · tsc/lint/vitest(604)/build 0. **이월(라운드 16, C가 순서 확정)**: 초원 4장면 바람 이식(`spring/summer/winter.ts`에 `wind` 소비자 **0개** — 1초 움직임 ×1.01인데 `land.ts` 계열은 ×2.4~4.6, 달력 뒤 화면이 이것) → 육지 정지 프레임 눕기 → 비의 젖음 채널(`world/surface.ts`) → 갯골 합류각·언덕 세 겹·암석 뭍 판. 다음 = **라운드 16**.
- **앰비언트 비주얼 QA 라운드 11 완료(2026-09-06, `docs/ambient/rounds/ROUND-11.md`)** — 자율 6라운드의 두 번째, 우선순위 E **안개 박스**. 세 Inspector 모두 한 뿌리("안개장이 지형을 모른다"). ① `world/fog.ts` 신설 — F(x,y) = 깊이항(원경 .55 → 근경 .05 단조) × 고도항(장면 `Scene.fogFloor(x, f)`: 언덕 `hillRidge(1,x)` · 산 `screeBand.top` · 계곡 최근접 유로 y · 민물 `waterTopAt` · 해안 물가선; 가까울수록 예민 v^.6) × 32×32 노이즈 윗변, 1/8 해상 굽기; 옛 `groundFog`(세로 그라데이션 + 고정 v .3 띠 + 바닥 8%f) 폐기; `depthFade`가 `currentLight().groundFog`로 F를 읽어 나무·생물·낙엽·소품 그림자 열 곳이 한 줄로 안개를 받음; 뭉치는 원·중경(v .04~.5)에서만, α .10~.18·속도 12~30·맥동; fog desat .2 → .06 ② 안개 하늘 — `bakeClouds` 안개 분기(낱개 원반 0, 저주파 얼룩), `drawSkyLive` 해·달 **halo만** ③ **라운드 10 회귀 둘**: 가로 광이 hz에서 풀 세기(절단선 −2.5~−5.1) → 세로 램프 8단 + 날씨 계수(`shadow.alpha`); 밤 지평선 −17L 뒤 산 ①이 땅 앵커로만 구워져 sky↔① 1.2/1.9 → `estimateSky(f)`(`skyPalette` hz × 오버레이 × far, resize·step 양쪽)로 천장 `max(anchor+22, min(81, 추정+6))` + 폭 세 단 고르게 → 밤 **9.8/6.0**, 새벽 8.8/10.9, 점심 불변 ④ 깊은 바다 후속(아침 기둥 3·dayK .6, 밤 글로우 ×2/×1.4). 실측: 발치 D 초원 .06·갯벌 .32·계곡 .34(0/7 → 2/7 ≤ .3) · 산 돌출 1.20 → .27 · 산 원경 D 1.79 → 3.07 · 민물 수면 .08 → .81(목표 ≥ 3 미달) · 단조 4/7(지표가 바탕 명도에 의존 — 12부터 잔차 밀도·실루엣 대비로). 회귀 0 · 깊은 바다 고유 해시 6 · selftest ALL PASSED · tsc/lint/vitest(604)/build 0. 조정 기록: 천장 오프셋 −7 → −2 → +6, 하한 +12 → +22(밤 폭 붕괴). 이월: 12 C 간격·레이어(B 부록 C: `world/spacing.ts`·풀 뒤 층·미경유 5루프) · 13 D 해안 뭍 클립 · 14 날씨 지면/수면/그림자 + 바람 + tint 채도 + 평면 잔차 지표 · 15 F/G/구름/섬/바코드/밤 ①↔②(구조). 다음 = **라운드 12**.
- **앰비언트 비주얼 QA 라운드 10 완료(2026-09-06, `docs/ambient/rounds/ROUND-10.md`)** — 소유자 부재 중 **자율 6라운드(10~15 + Final Visual Audit)**의 첫 라운드. 운영 규칙(소유자): Inspector 3인 동시·각 ≤ 5건, 메인 ≤ 2건(같은 뿌리 ≤ 3건), before/after/diff, 회귀 재검사, 라운드별 커밋 `fix(ambient): complete visual QA round NN`, 우선순위 A 시간대 > B 날씨 > C 간격·레이어 > D 물 접촉 > E 안개 박스 > F 갯골 > G 언덕 > H 해안 판 > I 잔여. 세 보고가 **한 뿌리**로 수렴: 조명이 전 화면 후처리(`drawLightPass` multiply·saturation·overlay)라 시간대·날씨·안개가 전부 "필터"(C 신지표 tintRes 초원 0.17% · 갯벌 0.03%, 언덕만 14~62%). 선택 3건(뿌리 A): ① **띠 프로파일** — `BAND_SKY` 밤 천정 "84 100 140" → "50 62 98"(저녁보다 밝던 "흐린 저녁"), 밤 지평선 흰색 → L* ≤ 80, 새벽 < 노을, 저녁 지평선 노을 잔광; `BAND` 저녁·밤 multiply 중성화(파란 multiply가 따뜻한 색 크로마를 상쇄해 채도 ×.39) + desat ↓, 아침 그림자 α 1.15 ② **빛의 방향을 지면에도** — `drawLightPass` 가로 multiply(해 쪽 그대로, 반대쪽 −10%, 세기 ∝ 그림자 길이) = 전역 affine으로 설명되지 않는 첫 공간 채널 ③ **깊은 바다 시간대**(A·B·C 동일 의견, 소유자 우선순위 A) — `deep.ts`가 `f.time.band`만 읽어 빛줄기 수·세기·기울기, 위 1/3 명도(바닥은 밤낮 같음), 밤 발광; `sealed()` 유지(날씨·계절), **계약 개정** "30해시 동일" → "띠마다 5날씨 동일(고유 해시 6)"(selftest ④·⑨ · GRAMMAR §11 · CLAUDE.md · ADR-0017 ⑯ 개정 · DECISION_INDEX). 실측: 천정 L 새벽 56.8 < 노을 62.1 · 저녁 46.0 > 밤 39.0 · 깊은 바다 고유 해시 6 · 날씨 봉인 ✔ · 위 띠 점심−밤 10.0 · 회귀 0. 크로마 비는 부분(남은 원인 = 청회 tint 오버레이 → 라운드 14). 도구: `spawn-probe HZ_V .26`(두 라운드째 지적), 프로브는 `captureCanvas` 비트맵(요소 스크린샷이 HTML 미니맵 패널을 함께 찍어 "비결정·날씨 누출"로 오판 — 교정). 게이트 tsc 0 · eslint 0 · vitest 604 · build 0 · selftest ALL PASSED. 이월: E 안개 밀도장(11) · C 풀 층 순서/claimSpot(12) · D 해안 뭍 클립(13) · B 날씨 지면·수면 채널 + 바람 + tint 채도(14) · F/G/구름/물풀 섬(15). 다음 = **라운드 11**.
- **앰비언트 비주얼 QA 라운드 9 완료(2026-09-06, `docs/ambient/rounds/ROUND-09.md`)** — 소유자 위임(6~10 자율)의 네 번째. **P0 하나**: 라운드 8에서 넣은 `drawWaterLight`의 **이중 평행이동**(전단 행렬 e항에 `x`가 있는데 다음 줄 `drawImage`가 이미 절대 x로 그렸다) — 노을엔 해와 **383px** 어긋나고 새벽·밤엔 띠가 화면 밖이라 라운드 4의 **달빛 길이 한 번도 보인 적이 없었다**(→ 8px). 입구 묶음 넷: ① **물 접촉을 한 함수로** — `drawSubmerged`의 밑단 페이드를 알파 0으로(민물 바위가 폭 65px 61/61열 수평 컷이었다), 젖은 띠를 몸통색 −10L로, **앞 반원 수면선을 함수 안에서** 소품 폭에 맞춰(호출부 링 셋 제거), 노두 깊이 ≤ 16px, 민물 물풀 섬 잠김 + 갈대를 섬 경계 안으로. ② **하늘 판의 띠 인식** — `light.ts`에 `tintTo` 신설(색조는 두고 밝기를 **그 띠 하늘 ± dL**에 맞춘다): 라운드 8이 판에서 고친 부호를 `lightOf()`의 상수 밝은 회색 오버레이가 한 겹 위에서 되돌리고 있었다(밤 흐림 판 −17.5L → 화면 −0.1L). 눈·비도 띠별 혼합으로, 노을을 어두운 버킷으로, 안개 구름 덩이를 천정에서(cover .3 → .12). ③ **풀 진행파를 2차원 격자 + 원근으로**(옛 판은 y를 안 읽어 원경이 근경보다 크게 움직였다) + 산 밀도. ④ **`softBlob`을 저해상 노이즈 마스크로** — 라운드 8의 동심 폴리곤 3겹이 땅 14장을 **등고선 지도**로 만들었다. 그 밖: 겨울 침엽수 기준색을 ③ 애추 띠로(+9.3L 밝던 것), 갯벌 바다 띠 22% → 규격, `shadowKey`에 조명 포함. **되돌린 것**: "띠를 아는 산 층 사다리" — `currentLight().sky`는 렌더된 하늘이 아니라 **오버레이 틴트**라 어두운 띠에서 ①이 내려앉았다(sky↔① 12~18 · ①↔② 0.8~3.3). 대신 ①↔②를 누르는 주범이 **`drawDepthHaze`**임을 확정(설계 12.5L → 화면 5.4L, ×0.43; 폭 31L로는 ≥ 8L이 산술적으로 불가능). 실측: 안개 Δ ≤ +4.86(12/12) · 안개 세로 6.31~18.29(12/12) · 비 Δ −8.47~−16.46(12/12) · 눈 Δ +16.49 → −5.53~−0.03 · 산 맑은 점심 1초 2,103 → 3,333px. 게이트 tsc 0 · eslint 0 · vitest 604 · build 0 · selftest ALL PASSED. 다음 = **라운드 10**(헤이즈가 산 ①을 누르는 문제 · 바람이 나무 없는 다섯 바이옴에 안 닿는 문제).
- **앰비언트 비주얼 QA 라운드 8 완료(2026-09-06, `docs/ambient/rounds/ROUND-08.md`)** — 소유자 위임(6~10 자율)의 세 번째. 주제는 **시간·날씨의 두 번째 채널 + 산 층 팔레트**. ① **그림자 채널**(C 3라운드 연속 P1): `light.ts` 새벽·저녁 `shadow.alpha` .55 → .80 · `propShadow` **면적 보존**(늘어난 그림자가 같은 알파로는 코어가 4L을 못 넘었다) · 신설 `drawCreatureShadow`로 생물도 소품과 같은 계약. ② **바람 삼단**: `drawTree`의 `wind ≥ .15` 계단 제거(진폭·주기·지연이 크기의 함수) · 풀포기를 **움직이는 층**으로(`bakeGrass`, 숲·언덕·산) · 물보라 × wind. ③ **입자 원근**(`depthOf`) + 밤 `mul` 184 → 194. ④ **산 층 팔레트를 구조 교체** — 상수 hex를 버리고 `labL`/`setLabL`로 **④(발치 땅) 앵커 + L\* 스텝**(① = min(앵커+31, 84) · ② = ① − 12.5 · ③ = max(앵커+9, ① − 23.5); 겨울은 눈 알베도 예외). 여기서 구조적 사실 하나가 나왔다 — **발치가 밝으면(봄·여름 L\* 58~64) 하늘까지 20~26L밖에 없어 ≥ 8L 단차 넷(32L)이 물리적으로 안 들어간다** → `GROUND.mountain`의 봄·여름을 가을 수준으로 어둡게. 층 안 사면 명암(`fg` 램프를 보이는 구간으로 + 대각 광원 α ×2)과 침엽수 실루엣의 지면 −7L 잠금까지. ⑤ **라운드 7 잔여를 닫는 하늘 판 수술**: 흐림 덮개를 지평선 회색 평면 → **그 y의 하늘색 −6L**(어두운 띠는 오프셋도 크게), 안개 천정을 **지평선이 아니라 천정에서** 만들기, 바람 새털구름을 near 층까지. ⑥ `land.ts`의 해·달·별을 지평선 띠 **앞**으로(라운드 7 수정이 초원 계열에만 들어가 있었다) · **버섯 회귀 수정**(앵커 계산이 나무 배치보다 먼저 돌아 0개였다) · 여울 "차선 대시" 분산 · 물 위 빛의 길 14행 사다리 → 스프라이트 한 장 · 봄 참나무 채도 .92 → .45(`scripts/ambient-art-satcap.mjs`) · `selftest`에 봉인 장면 분기. 실측: 흐림 6띠 [+7.5…+14.7] → [−3.3…−0.7] · 안개 하늘 세로 1.5~3.0 → **6.2~18.4** · 여름 산 ①↔② 3.9 → **6.4~10.6** · 지평선 아래 1초 변화 0px → **1,539~16,398px** · 언덕 마루 아래 해·별 픽셀 **0** · 가을 숲 버섯 0 → **161 크림 픽셀**. 게이트 tsc 0 · eslint 0 · vitest **604** · build 0 · `ambient-qa/selftest` ALL PASSED. 다음 = **라운드 9**(첫 입구 = `drawSubmerged` 닫힌 타원 수면선 3바이옴 · 띠 틴트를 곱셈 → 덧셈 · 산 층 측정창에서 나무 제외 · 갯벌 바다 띠 28.5%).
- **앨비언트 비주얼 QA 라운드 7 완료(2026-09-06, `docs/ambient/rounds/ROUND-07.md`)** — 소유자 위임(6~10 자율)의 두 번째. ① **물가 접촉 묶음**(라운드 6 B가 올린 6건): 조류대·물보라·표착선이 물가 곡선을 따라가게(묍 판을 열 단위로 밀어 굽고 `shoreOffsetAt` 하나로 통일) · 물가 노두 `drawSubmerged` · 갯벌 물골 교차 재추첨·바늘 끝 제거·획을 물 밖으로 클립 · 계곡 물 안 바위(수면선·후류 공식 수정) · 민물 물풀 섬 모래톱 · 거품 마디. ② **간격**: `claimSpot(stand)` — 서 있는 것은 눌리지 않은 화면 세로 간격을 따로 본다 · 버섯은 나무·그루터기 90px 안 군집 + 발밑 그림자. ③ **구름 흐름**(결정 4): 하늘 판/구름 타일 분리, 폭 2w 주기 w로 이음솔 0, 날씨·고도층별 속도. ④ **fixture `m`/`day` + `--kinds moon`**(결정 5). ⑤ **라운드 6 회귀 셋**: 산 봉우리를 지평선 **위**로 복원(A 실측 — 초원 먼 언덕이 산 정상보다 100px 높았다; 산 다섯 층 ①↔② 2.3 → **6.9**) · 아트 윤곽 채도 상한을 모든 색조에(봄·여름 오크 19%가 채도 1.00 청록이었다) · 해·달·별을 지평선 띄보다 먼저 그려 언덕이 가리게 · 흐림 하늘 부호 교정 · 안개 하늘 세로 폭 · 겨울 ③ 전폭 실선 제거 · 침엽수 두 어법 분리 · 심해 z 정렬 통합. 신규 **P0**(민물 기슭 소품이 물 안 30~33px — 상수를 두 곳에 따로 적은 탓)은 `shoreLandEdge` 공유로 **부분 닫힘**. 게이트 tsc 0 · eslint 0 · vitest 603 · build 0. 깊은 바다 봉인 11장 해시 동일 유지. 다음 = **라운드 8**(시간·날씨의 두 번째 채널 + 산 층 팔레트 + 측정창 통일).
- **앨비언트 비주얼 QA 라운드 6 완료(2026-09-06, `docs/ambient/rounds/ROUND-06.md`)** — 소유자가 자기 전 남긴 지시 묶음 + "라운드 6~10은 알아서, 열린 결정은 에이전트 3인 의견 종합으로" 위임. ① **하늘 비율**: `HORIZON_V` .12 → **.26**(A .30 / B .20 / C .26의 공통 구간) — 좌표 서비스 신설(`groundYAt` 땅 비율 · `aboveHz` 지평선에서의 거리 · `groundK` 절대 px 보정)로 먼 언덕·나무 줄·안개 시작·지평선 광은 거리로, 물가·기슭·산 진폭·소품 개수는 땅 비율로 재정의. 해안은 `LAND_V_BY`(화면 분수) → `SEA_GV_BY`(땅 비율), 산은 **지평선 아래 유지**(봉우리가 넘으면 안개 띄를 뚫어 대기 원근이 뒤집힌다). 하늘 내용도 같이 — 구름 세 고도층 · 알파 양자화(픽셀 계단) · 새털 갈고리 획 · 별 밀도·밝기 3등급 · **은하수** · 해·달 저해상 굽기 · 고도 재배치(별 hz·.92 · 달 .35 · 해 .80 — 옛값은 전부 상단 31px). 가시 하늘 3~5% → **23~36%**. ② **깊은 바다 = 물속 옆모습**(신규 `scenes/deep.ts`): 지평선·바닥 누름·거리 축소 없음, y = 수심, 원근은 개체 z, 물고기는 **옆면** Noto 실루에(8종 신규 내려받음), 바닥 없음. 엔진 계약 `Scene.sealed()` — 입자·대기 안개·조명 패스를 통째로 건너뛴다(팬 중에는 봉인 해제). **띄 6 × 날씨 5 = 11장 해시 전부 동일**로 계절·시간대·날씨 무영향 증명. ③ 납품 아트 **순검정 윤곽선 제거**(`scripts/ambient-art-outline.mjs`, 오크 4장 최저 L 0.4~4.7 → 13.2~15.5). ④ **눈 편한 테마 분리**(소유자): 루트 `filter` 폐지 → 토큰 팔레트 한 벌로만 — 배경 캔버스는 이제 테마와 무관하고, 알려진 프레임 끊김 원인 하나도 사라졌다. **열린 결정 5건이 전부 닫혔다**(밤 산 반전 — 미채택, 간격 재분배로 · 항등 계약 — 3층으로 재정의 · 하늘 색 — 세로 변화에 예산 · 구름 흐름 — 채택 · fixture m/day — 채택). 게이트 tsc 0 · eslint 0 · vitest 603 · build 0. 다음 = **라운드 7**(물가 접촉 묶음 · 간격 · 구름 흐름 · fixture m/day).
- **팬 타임라인 파서 전수조사·확장(2026-09-06, CHG-20260906-001)** — 사용자 신고("ㄴ로 매단 세부 항목이 표시가 안 된다")에서 출발해
  **VOD 385편 댓글 전수조사**. 진짜 원인은 더 컸다: 시각을 대괄호로 감싸는 표기(`[ 02:10:55 ] 라벨`)를 파서가 못 읽어 **그런 댓글은 통째로**
  버려지고 있었다(항목 0 = "타임라인 없음"). 받아들이는 표기를 넓혔다 — 괄호 시각, 계층 표시 ㄴ/└/>(앞·뒤 양쪽, 라벨에서 떼어 `depth`로),
  시각+라벨 붙임(`01:18:00토리님 복귀`), 시각 뒤 구분 콜론, 분·초 한 자리 오타, **라벨이 앞·시각이 뒤**(`발로란트 01:23:25`, `[🌺무꽃피] 54:30`,
  한 줄 여러 시각 = 항목 여러 개). 일부러 안 받는 것 = 라벨 없는 시각 나열(`4:40:04*` 76줄, 클립 표시), 날짜 표기, 32자 넘는 문장(다른 방송 지목).
  파서 거울 `scripts/lib/timeline-parse.mjs`(+`.d.mts`)를 만들어 백필 스크립트의 사본 표류를 없앴고, **거울 동치 테스트**가 두 구현을 붙잡는다.
  전량 재수집 결과 복구 1편·증가 64편·항목 +938개·계층 항목 3,462개, 손실 0. UI는 `depth`를 들여쓰기로만 표현(글리프 없음).
  남은 것: 없음(잔여 탈락 2줄은 의도적 제외).
...
- **감상 중 배경 설정 톱니(2026-09-05, 소유자 요청)** — 감상 모드는 크롬을 전부 감춰 설정으로 갈 길이 없었다(나가기 →
  ...
  **계절 · 시간대 · 날씨 · 배경 효과**를 보면서 바꾼다. 값은 설정 모달과 **같은 상태**(studio-shell의 `devSeason` ·
  `devWorld` · `gfxPref`)라 두 자리가 어긋나지 않는다. 시간대·날씨는 `forceWorld`로 캔버스를 다시 굽지 않고 바로
  들어가고(빛은 3초 lerp), 계절만 캔버스를 다시 만들되 **지금 서 있는 바이옴을 `force.biome`에 실어** 그 자리에서
  계절만 갈린다. 개발자 역할에만 붙는다(설정 모달의 개발자 줄과 같은 규칙 — 미리보기 중인 역할엔 안 보임). 판이 열려
  있으면 Esc는 판만 닫는다. 유리 문법 + 세그먼트 공통 토큰(`--seg-on-*`), 칩은 줄이 고르게 나뉘는 그리드.
  검증: tsc·eslint(0 경고)·vitest 574·build 통과 + fixture 실측(감상 진입 → 톱니 → 겨울·밤·눈 즉시 반영,
  Esc 2단, `__vicAmbient.world()` = night/snow).
  **후속(같은 날): 설정 모달에 '월드 계절(개발자)' 줄 추가** — 감상 중 봄으로 바꾸고 나오니 편집실 배경이 봄에
  갇혔다(소유자 보고). 감상 톱니 밖으로 살아남는 강제값은 **설정 모달에도 손잡이가 있어야 한다**는 규칙으로 정리.
  `dev-world-season`(자동/봄/여름/가을/겨울), 계절을 강제하면 날씨 목록도 그 계절 것(양쪽 동일).
  실측: 10월(가을) → 봄 강제 → `data-season=spring` → 자동 → `autumn` 복귀.
...
- **'오늘' 표식 관례화 + 링을 띠 위로(2026-09-05 소유자 결정, 둘 다)**: 관례 조사 결과 Google·Apple·
  ...
  칸 크롬 위에 그린다. ① 숫자에 금색 알약 배지(시청자 `.d-num`, 편집실 `.studio-day-head strong`).
  띠는 머리줄 **아래 레인**에 깔리므로 배지는 무엇에도 안 가려진다. 배지 높이는 머리줄 안쪽 높이로 묶어
  (시청자 21px·편집실 18px) 머리줄이 안 커지게 했다 — 커지면 띠 레인과 카드가 통째로 밀린다
  (실측: 머리줄 21→21, 20→20 불변, 배지 아래끝 21px vs 띠 위끝 22px).
  ② 링도 소유자 요청대로 띠 위로 — `--z-cal-select-ring` 5 → **8**(띠 6·머리 7 위). 오늘 칸을 지나는
  띠에 금색 실선이 그어지는 건 감수한 거래. 시청자 머리줄의 금색 그라데이션은 뺐다(배지와 두 겹이면 시끄럽다).

- **계절 배경 카드 순간 이동의 진짜 원인 · 띠 vs 오늘 링(2026-09-05)**:
  ① 문제는 카드가 아니라 **아바타 자리**였다. 시청자 화면에서 이 카드는 rail이 아니라 `.avatar-slot`
  안에 산다(좌↔우로 옮겨 다니는 그 칸). 그 칸의 등장 애니메이션 이름이 `avatar-rise` **하나**라
  side를 바꿔도 다시 돌지 않고 left/right 좌표만 바뀌어 한 프레임에 반대편으로 갔다. 옆 카드들은
  다른 컨테이너(.public-right · .avatar-side-rail)에 있고 그쪽은 side별 이름이라 매번 다시 돌았다 —
  그래서 그 카드들만 살아 있었다. 이름을 `-l`/`-r`로 가르고 keyframes는 동일하게(연타 순간 이동 방지).
  실측: 이름 둘 다 등장, translateY 24 → −5(오버슈트) → 0, opacity 0 → 1.
  ② 오늘 링 vs 업 도움·기간 안내 띠 — 편집실은 이미 "띠가 위"로 정해 두었는데(끈이 잘려 보인다는
  실측 주석) 시청자 화면만 반대였다(둘 다 z:4 → ::after가 이김). 시청자도 띠 6 / 머리 7로 통일.

- **rail 카드 착지의 12px 순간 이동 제거(2026-09-05)**: "처음 한 번은 괜찮은데 여러 번 누르면 툭툭"의
  정체. 착지 애니메이션은 side로 이름이 갈려(`-l`/`-r`) 다시 돌지만, `both` + delay 0.28s 구간에서는
  'from' 값이 그대로 걸려 있다 — 그 값이 방향마다 `translateX(∓6px)`이라 연달아 누르면 걸린 값이
  +6 → −6으로 **한 프레임에 12px** 튀었다(프레임 단위 실측: 5번 누르는 동안 12px 점프 5회).
  두 keyframes를 **방향에 안 걸리는 같은 값**(scale .982 + opacity .9)으로 바꿔 걸린 값이 늘 같게 했다 —
  점프 12px → **0px**, 스프링 바운스(scale 0.982→1.004→1)는 그대로, 세 카드 매 프레임 동일.
  (참고: 카드가 이웃과 다르게 움직인다는 가설은 세 가지 방법으로 계측해 모두 부정됐다 — 원인은
  카드 고유가 아니라 세 카드 공통의 '걸린 값' 불연속이었고, 작아서 눈에 먼저 띈 것이 계절 배경 카드였다.)
...
- **스크롤바 전면 제거 · 단축키 안내바 정렬(2026-09-05)**: ① 화면·태그 필터·rail·모달을 하나씩 막아
  ...
  실측: 시청자 화면·이 달 기록 시트·편집실·인사이트 창에서 none 아닌 곳 0, 문서 gutter 0.
  ② 단축키 안내바를 **달력 패널 안**으로 이사 — 셸의 형제로 두고 margin(18 + avatar-scene 20vw)으로
  폭을 흉내 내다 보니 왼쪽 끝이 12px 어긋나 있었다. 이제 요일 줄과 **같은 상자**(x·right 일치 실측).
  칸이 12px 좁아진 만큼 칩 간격을 8 → 7로 줄여 한 줄을 되찾았다(≥1920px에서 44px 한 줄).
  검증: tsc 0 · lint 0 · vitest 559 · build exit 0 · probe-sb 5항목 통과.
