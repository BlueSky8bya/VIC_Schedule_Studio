...
- Entity reference images support subject form, structure and ecology; they are not approved style. `공통화풍참고/<게임>/` supports overall mood, color, composition, camera and how the source game simplifies the same kind of subject; its `색인.json` decides per image whether `request` may attach it (ART-14, ADR-0021). Image bytes and filenames there stay as the owner placed them; classification lives only in the index. Approved project sheets remain style authority. Record images actually inspected and passed to generator; only request-listed frozen inputs establish that run's reference set.
