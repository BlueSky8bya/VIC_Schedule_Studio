...
export function createShallow():Scene {
 const image=new Image();let pending=true,loaded=false,disposed=false;
 beginLoad();image.src='/ambient/art/concept-shallow-v1.png';
 ...
 return {
  ...
  draw(g,f){
   g.fillStyle='#65b7cf';g.fillRect(0,0,f.w,f.h);
   if(loaded){
  ...
  debug:()=>({biome:'shallow',concept:true,pending,loaded}),
