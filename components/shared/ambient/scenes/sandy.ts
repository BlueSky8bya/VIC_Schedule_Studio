import {RockWaterMask} from '../world/rock-water-mask';
...
export function createSandy(seed:number,season:SeasonKey,biome:'sandy'|'rocky'='sandy'):Scene{
 const backdrop=new HillBackdrop(season,biome),relief=new ReliefLayers(),surface=new SandySurface();
 const waterMask=biome==='rocky'?new RockWaterMask():undefined;
 ...
 const bake=(f:Frame)=>{
  w=f.w;h=f.h;
  if(!backdrop.ready||waterMask?.pending)return;
  ...
  if(maskCanvas)maskCanvas.width=maskCanvas.height=1;maskCanvas=waterMask?.bake(w,h);
 };
 return {
  resize:bake,step(f){bake(f);const target=.65+Math.abs(f.light.wind)*.4;waveStrength+=(target-waveStrength)*(1-Math.exp(-f.dt*1.4));},
  ready:()=>!backdrop.pending&&!waterMask?.pending,
  draw(g,f){
   ...
   if(terrain){
    withReliefSurface(g,(x,y)=>usingGPU?sandyMotion(backdrop.sourcePoint(x,y,w,h)?.v??0):reliefMotion(distance(x,y),reliefLayerCount(f.depthTier??'full')),(off,tier)=>{
     ...
     usingGPU=!!top&&!!bottom&&surface.draw(g,terrain!,w,h,off,tier,f.t,strength,{y:top.y,height:bottom.y-top.y},maskCanvas);
  ...
  debug:()=>({biome,season,backdrop:backdrop.debug(),relief:relief.debug(),water:surface.debug(),leaves:0,creatures:0,terrainBytes:terrain?terrain.width*terrain.height*4:0}),
  dispose(){backdrop.dispose();relief.dispose();surface.dispose();waterMask?.dispose();if(maskCanvas)maskCanvas.width=maskCanvas.height=1;if(terrain)terrain.width=terrain.height=1;if(sky)sky.width=sky.height=1;terrain=sky=undefined;clouds=null;},
 };
}
