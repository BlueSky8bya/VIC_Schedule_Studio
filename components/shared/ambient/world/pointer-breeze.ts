 ...
 */
export function pointerBreeze(body:{x:number;y:number;vx:number;vy:number},f:Frame,size:number):number {
  const p=f.p,sp=Math.min(2600,Math.hypot(p.vx,p.vy));
  if(f.dt<=0||!p.inside||sp<=30)return 0;
  const dx=body.x-p.x,dy=body.y-p.y,d=Math.hypot(dx,dy),radius=110+60*f.load+size*.6;
  if(d<=.001||d>=radius)return 0;
  const k=(1-d/radius)*(.35+.65*f.load)*meadowSpeed(body.y,f.h),nx=dx/d,ny=dy/d;
