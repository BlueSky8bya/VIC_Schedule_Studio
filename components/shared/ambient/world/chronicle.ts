...
export const CHRONICLE_EPOCH = 2024; // 이 해 가을부터 세계가 시작된다
export const TREE_CAP = 6;
export const TREE_LIFESPAN = 6; // 나무는 여섯 주기 서 있다가 떠난다 — 상한이 차도 세대가 돌아 숲이 멈추지 않는다

export type TraceKind = "cache" | "sprout" | "sapling" | "tree" | "molehill" | "snowman" | "lilypad";
export type Trace = {
  kind: TraceKind;
  id: string;
  u: number; // 0~1 정규화 가로
  v: number; // 0~1 정규화 세로
  /** 나무: 나이(년, ≥1) · 눈사람: 단계(1~3) · 흙더미: 0 흙 / 1 풀 얼룩 · 연잎: 크기 배율 · 그 외 0 */
  stage: number;
  /** 태어난 주기(가을 시작 연도) */
  cycle: number;
};

// 띠 위치 — 위(50%) · 아래(30%) · 왼쪽(10%) · 오른쪽(10%). 나무는 위 띠만.
function bandPos(r: () => number, topOnly = false): [number, number] {
  const k = topOnly ? 0 : r();
  if (k < 0.5) return [0.04 + r() * 0.92, 0.02 + r() * 0.08];
  if (k < 0.8) return [0.04 + r() * 0.92, 0.9 + r() * 0.08];
  if (k < 0.9) return [0.02 + r() * 0.08, 0.14 + r() * 0.72];
  return [0.9 + r() * 0.08, 0.14 + r() * 0.72];
}
