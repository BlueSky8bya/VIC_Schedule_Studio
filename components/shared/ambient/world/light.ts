...
export function lightOf(band: DayBand, weather: Weather, season: SeasonKey): Light {
  ...
  switch (weather) {
    ...
    case "rain":
      ...
      //
      // **탈채도의 범인은 하늘 베일이 아니다**(2026-09-07 라운드 17, 검토 C 실측). 라운드 16은 근경의 회색을
      // 전면 하늘 오버레이 탓으로 지목했는데, 그 오버레이가 만드는 far-band는 `groundYAt(.30)`에서, 안개는
      // `hazeEndY`에서 끝난다 — h=860이면 y 415·402로, **화면 아래 절반에 물리적으로 닿지 않는다**. 맑음 프레임에
      // 아래 mul·앵커만 적용한 예측값이 실제 비 프레임과 소수 둘째 자리까지 맞는다(언덕 v .45~.70 예측 L52.4/C9.59
      // vs 실측 L52.4/C9.58). 즉 근경을 회색으로 만드는 것은 **채널 비대칭 곱셈** 하나다: [.84,.86,.90]은 R을 B보다
      // 6% 더 눌러 따뜻한 색(가을 갈색·마른 억새)의 R−B를 깎는다. 라운드 10이 저녁 mul에서 이미 진단한 그 항이다.
      // 휘도만 남기고 색온도는 위의 하늘·틴트에 맡긴다 — 실측 크로마비(비÷맑음) .634 → .774, 앵커까지 얇게 하면 .823.
      L.sky = mixRgb(L.sky, tintTo(L.sky, "132 142 154", -48), 0.75); // 덮는 넓이 대신 색판으로 −14L을 낸다
      // 하늘 오버레이 .38 → .28: 이 값은 원경 far-band(α = skyAlpha × 0.6)와 함께 얇아진다(.228 → .168).
      // 원경 크로마가 맑음의 0.20까지 깎이던 것(언덕 C 11.76 → 2.37)이 여기서 나왔다.
      L.skyAlpha = Math.max(L.skyAlpha, 0.28);
      L.mul = scaleMul(L.mul, [0.86, 0.86, 0.86]);
      L.ground = { rgb: L.ground.alpha ? L.ground.rgb : "44 50 62", alpha: L.ground.alpha + 0.05 }; // 젖은 땅은 어둡다 — 단 ΔL −8 목표에 맞게 얇게(라운드 17)
      // 비의 지면은 **젖어서 진하다** — 탈채도는 규칙의 반대 방향이다(2026-09-06 라운드 9, 검토 C:
      // Δ채도 −0.05~−0.07, 규칙 +10%). 원경 대비 −35%는 아래 `hazeK`가 이미 맡는다.
      // (젖음으로 채도를 **올리는** 것은 웅덩이 반사와 함께 별건으로 남긴다 — 백로그. 근경이 베일 밖임이
      //  증명됐으므로 전 화면 블렌드가 아니라 지면·소품의 국소항이어야 한다 — 라운드 16이 소진한 셋을 다시 시도하지 말 것.)
      L.shadow.alpha *= 0.3;
      L.glint = 0;
      L.reflect.k = 0;
      L.reflect.skyK = 0;
      L.hazeK *= 1.15; // ×1.3 → ×1.15: −35% 대비 저하는 far-band가 이미 대부분 낸다(이중 계상이었다)
