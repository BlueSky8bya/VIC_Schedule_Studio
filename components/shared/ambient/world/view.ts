...
export function drawLightPass(g: CanvasRenderingContext2D, w: number, h: number, L: Light, fogFloor: ((x: number) => number) | null = null, fogFloorKey = "", skipFog = false) {
  const hz = horizonY(h);
  const pad = depthPadding(g);
  g.save();
  if (L.groundFog > 0 && !skipFog) {
  ...
  if (L.skyAlpha > 0) withDepthLayer(g, "sky", () => {
    ...
    if (L.reflect.skyK > 0.005) {
      ...
      // Horizontal lighting must also fade vertically: an unmasked rectangle
      // left a full-width night-sky edge at end, above low mountain saddles.
      const key = `sky:${stops.map(([p,c])=>`${p.toFixed(3)}:${c}`).join('|')}`;
      let sheet = directionCache.get(key);
      if (!sheet) {
        sheet=document.createElement('canvas');sheet.width=256;sheet.height=256;
        const sg=sheet.getContext('2d')!,horizontal=sg.createLinearGradient(0,0,256,0);
        for(const [p,c] of stops)horizontal.addColorStop(p,c);
        sg.fillStyle=horizontal;sg.fillRect(0,0,256,256);
        const fade=sg.createLinearGradient(0,0,0,256);
        for(let i=0;i<=16;i++){const t=Math.max(0,(i/16-.45)/.55);fade.addColorStop(i/16,`rgba(0,0,0,${1-t*t*(3-2*t)})`);}
        sg.globalCompositeOperation='destination-in';sg.fillStyle=fade;sg.fillRect(0,0,256,256);
        directionCache.set(key,sheet);
        if(directionCache.size>4){const oldKey=directionCache.keys().next().value!;const old=directionCache.get(oldKey)!;directionCache.delete(oldKey);old.width=old.height=1;}
      }
      g.drawImage(sheet,-pad,-pad,w+2*pad,end+pad);
    }
  });
