...
export function BiomeFixture({ season, year, month, force, t, camera, gfx = "max", reduced = false, live = false }: BiomeFixtureProps) {
  ...
  const [previewSeason,setPreviewSeason]=useState<SeasonKey|null>(season);
  const [previewGfx,setPreviewGfx]=useState(gfx);
  const effectiveSeason=previewSeason ?? ((previewForce.month ?? month ?? SEASON_MONTH[season])<=2 || (previewForce.month ?? month ?? SEASON_MONTH[season])===12 ? 'winter' : (previewForce.month ?? month ?? SEASON_MONTH[season])<=5 ? 'spring' : (previewForce.month ?? month ?? SEASON_MONTH[season])<=8 ? 'summer' : 'autumn');
  ...
  useIsoLayoutEffect(() => {
    setAmbientMode("on");
    setGfxPref(previewGfx);
    setReduceMotion(reduced);
    if (camera === "showcase") enterShowcase();
    return () => exitShowcase();
  }, [camera, previewGfx, reduced]);
  useEffect(() => {
    let cancelled = false;
  ...
  }, [t, effectiveSeason, forceKey, camera, previewGfx, reduced, live]);
  return (
    <>
      <AmbientLayer force={previewSeason} month={month ?? SEASON_MONTH[season]} slug="vic" worldForce={previewForce} year={year}/>
      {camera === "showcase" ? <ShowcaseExit settings={live ? {
        year,month:month ?? SEASON_MONTH[season],seasonForce:previewSeason,
        onChangeSeasonForce:value=>{setPreviewForce(prev=>({...prev,biome:window.__vicAmbient?.biome() ?? prev.biome}));setPreviewSeason(value);},
        world:{force:previewForce,onChange:setPreviewForce},gfxPref:previewGfx,onChangeGfxPref:setPreviewGfx
      }:undefined}/> : null}
    </>
  );
}
