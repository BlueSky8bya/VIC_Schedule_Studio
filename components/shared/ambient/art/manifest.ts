...
- **⚠ 레퍼런스 시트를 못 받았으면 그리지 말고 먼저 달라고 하라**(2026-09-09 추가). 이 의뢰에는 \`docs/ambient/reference/<자리>.png\`
  가 **첨부돼야 한다** — 합격본을 나란히 놓고, 밑동과 (겨울이면) 눈이 앉은 자리를 확대해 놓은 한 장이다.
  왜 이 문단이 생겼나: 여태 이 문서는 "합격본 파일을 열어 보고 맞춰라"라고 썼는데 **생성기는 그 파일을 볼 수 없었다.**
  그래서 그 지시는 빈칸이었고, 3차 납품이 그 빈칸을 제 나름대로 메웠다 — **수치로 재는 항목 여덟은 전부 통과하고**
  (도트 결·고아·얼룩·외곽선·다양성·계절 짝·눈의 양·잎 색상), **말로만 적힌 것 둘이 틀렸다**: 눈을 단 위에 얹지 않고
  윤곽에 흰 테를 둘렀고, 밑동이 게 다리처럼 세 갈래로 갈라졌다. 보지 못한 것은 지킬 수 없다.
...
- **첨부물은 두 종류이고, 권한이 다르다**(2026-09-09). 어느 쪽이 이기는지 헷갈리면 외부 그림이 우리 세트를 밀어낸다.
  ① **합격본 시트**(\`docs/ambient/reference/<자리>.png\`) = **정본.** 도트 굵기 · 색 단수 · 외곽선 · 단순함 ·
     밑동과 눈처럼 부위가 붙는 방식 — 새 그림이 **이것과 한 벌로 보여야** 합격이다.
  ② **바깥에서 가져온 참고 그림**(있을 수도, 없을 수도 있다) = **발상만.** 나무의 생김새·실루엣의 아이디어·
     단이 앉은 모양 같은 **형태의 힌트**로만 쓴다.
  ...
  ⚠ **②를 베끼거나 트레이스하지 않는다.** 남의 그림을 옮겨 그린 것은 이 프로젝트에 못 쓴다(ADR-0019). 형태의 발상을 얻되
...
4. **부위가 제 모양을 잃는 것** — 색이 옆 부위로 넘어가거나(줄기가 잎 색), 형태 자체가 딴 것이 되는 것.
   ⚠ 3차 납품에서 **밑동이 게 다리처럼 서너 갈래로 벌어져 내려왔다**(합격본은 한 덩이다). 나무는 **하나의 줄기**로
   땅에 닿고, 뿌리목은 그 줄기가 밑에서 살짝 넓어지는 것이지 따로 뻗은 다리가 아니다. 우리는 맨 아랫줄이 몇 갈래인지 센다 —
   **합격본과 같은 갈래 수**여야 한다.
   부위마다 **팔레트를 나누고**(줄기 = 갈색 계열 3톤, 잎 = 초록 계열 3톤), 한 부위의 색을 다른 부위에 쓰지 않는다.
   경계에서 섞어 부드럽게 만들려 하지 말 것 — 픽셀아트의 경계는 **딱 끊기는 것**이 정상이다.

5. **얹는 것을 두르지 말 것**(2026-09-09 추가) — 눈·이끼·서리처럼 **위에 쌓이는 것**은 물체의 *윗면을 덮는* 것이지
   윤곽을 따라 흰 선을 두르는 것이 아니다. 3차 납품의 겨울 소나무는 실루엣 가장자리에 흰 테를 둘러 후광처럼 보였다 —
   눈의 양(몸의 20% 이상)은 그렇게 해도 채워지므로, 우리는 **눈 칸이 가장자리에 얼마나 몰렸는지**를 따로 잰다
   (합격본 20.8%, 3차 납품 38~48%). 눈은 단의 **안쪽으로** 두툼하게 들어와야 한다.
...
아래 표의 **파일 이름마다 그림 한 장씩**을 만들어 달라 — 스타일은 **첨부된 합격본 시트**와 같은 픽셀아트다(그 시트가 이 의뢰의 정본이다).
...
- **픽셀아트 한 어법으로**: **첨부된 합격본 시트**와 나란히 놓아도 한 세트로 보여야 한다(다 그린 뒤 시트 옆에 붙여 놓고 판정할 것).
  시트가 첨부돼 있지 않으면 그리지 말고 먼저 달라고 한다 — 못 본 것은 맞출 수 없다.
