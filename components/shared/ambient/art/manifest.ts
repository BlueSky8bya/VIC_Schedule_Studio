...
const PHASE1: readonly ArtSlot[] = [
  ...
  // 풀·꽃
  { id: "grass-tuft", nameKo: "풀포기", nameEn: "Grass tuft", category: "plant", seasons: ["spring", "summer"], view: "stand", px: [22, 18], variants: 6, perScreen: 34, brief: "잔디 위에 솟은 풀포기 — 잎 5~7가닥이 부채꼴로 벌어지고 끝이 살짝 휜다. 밝은 초록, 뿌리 쪽은 어둡다. 변형 3개는 가닥 수와 휜 방향이 다르다.", acnhRef: "잡초(풀)", now: "procedural", phase: 1 },
  { id: "grass-tall", nameKo: "키큰 풀(강아지풀)", nameEn: "Tall grass / foxtail", category: "plant", seasons: ["summer"], view: "stand", px: [26, 46], variants: 6, perScreen: 20, brief: "무릎까지 오는 여름 풀 한 포기 — 길고 활처럼 휜 잎 4~6가닥 위로 **고개 숙인 원통 이삭**(강아지풀) 한두 대가 솟는다. 짙은 청록 초록에 이삭만 옅은 황록. 변형 3개는 이삭 수와 휜 방향이 다르다.", acnhRef: "여름 풀숲", now: "procedural", phase: 1 },
  { id: "grass-dry", nameKo: "마른 풀", nameEn: "Dry grass", category: "plant", seasons: ["autumn", "winter"], view: "stand", px: [22, 18], variants: 6, perScreen: 34, brief: "시든 풀포기 — 황토·밀짚색 가닥이 옆으로 누웠다. 변형 3개.", acnhRef: "잡초(가을)", now: "procedural", phase: 1 },
  ...
  { id: "mushroom", nameKo: "버섯", nameEn: "Mushroom", category: "plant", seasons: ["autumn"], view: "stand", px: [20, 22], variants: 4, perScreen: 8, brief: "갈색 갓에 크림색 점이 몇 개 찍힌 통통한 버섯(빨간 독버섯 금지 — 밤색·황토). 변형 2개: 하나는 작은 두 송이가 붙었다.", acnhRef: "버섯(가을)", now: "procedural", phase: 1 },
  { id: "reed", nameKo: "갈대·억새", nameEn: "Reed / silver grass", category: "plant", seasons: ["summer", "autumn", "winter"], view: "stand", px: [30, 70], variants: 6, perScreen: 12, brief: "물가에 선 갈대 서너 대 — 긴 잎과 갈색 이삭, 살짝 휘었다. **변형 4개**(자리마다 대의 수·키·휜 방향이 달라야 한다 — 물가에 수십 대가 서므로 같은 실루엣 복제는 바로 벽지로 읽힌다). 이번 배치는 표의 파일만.", acnhRef: "물가 풀", now: "procedural", pilot: 2, phase: 1 },
  // 물
  { id: "lilypad", nameKo: "연잎", nameEn: "Lily pad", category: "water", seasons: ["summer"], view: "flat", px: [56, 56], variants: 6, perScreen: 12, brief: "위에서 본 연잎 — 둥근 잎에 V자 갈라짐 하나, 잎맥이 가운데서 퍼지고 가장자리가 살짝 말려 올라 **두께가 보인다**(가장자리에 옅은 밝은 테 + 아래쪽 어두운 띠). 변형 3개는 갈라진 방향·크기가 다르다.", acnhRef: "연못 연잎", now: "procedural", phase: 1 },
  ...
  { id: "pebble", nameKo: "조약돌", nameEn: "Pebble", category: "ground", seasons: ALL, view: "flat", px: [12, 9], variants: 6, perScreen: 20, filler: true, brief: "위에서 본 둥근 조약돌 — 회색·밝은 회갈색, 위쪽에 작은 하이라이트. 변형 3개.", now: "procedural", phase: 1 },
  { id: "rock", nameKo: "바위", nameEn: "Rock", category: "ground", seasons: ALL, view: "stand", px: [40, 30], variants: 8, perScreen: 12, brief: "무릎 높이의 바위. **변형 4개는 실루엣이 서로 달라야 한다** — ① 넓적(가로로 퍼진 판) ② 길쭉(세로로 선 덩이) ③ 모난(각진 면 3~4개, 깨진 단면) ④ 둥근(공에 가까움). 색만 바꾼 복제 금지.\n\n**표면은 넷이 다 달라야 한다 — 이끼는 하나에만.** 이 바위는 사철 열한 바이옴에 전부 깔린다: 마른 모래해안·암석해안·겨울 눈밭·산 능선에도 놓이므로, **네 장 다 이끼가 앉아 있으면 그 화면에서 전부 틀린 돌이 된다**(2026-09-08 소유자 지적). 배분: ① 맨돌(이끼 0, 돌색만 3단) ② 맨돌 + 갈라진 금 1~2줄(그늘로만, 검은 선 금지) ③ 지의류 얼룩 — 회백·연회록 점 서넛, 이끼 아님 ④ **여기만 이끼** — 윗면 한쪽 귀퉁이에 작게(면적의 1/5 이하), 돌을 덮지 않는다.\n\n**도트 예산 = 가로 약 16칸**(자리가 작다 — 소나무는 35칸이다). 이 안에서 **반드시 살아야 하는 것 셋**: ⓐ 실루엣(넷이 구별되는가) ⓑ 면의 명암 3단(위 왼쪽이 밝고 아래 오른쪽이 그늘 — 이게 없으면 색 얼룩이 된다) ⓒ 표면 특징 1가지(위 배분). 그 밖의 잔 디테일은 버린다. 16칸이 적어 보여도 **면을 지우지는 말 것** — 단순화는 덩어리 수를 줄이는 것이지 입체를 없애는 게 아니다.\n\n⚠ **1차 납품 반려 사유 넷**(2026-09-08 실측 — 같은 실수를 반복하지 말 것):\n① **그늘이 몸을 잡아먹었다.** 가장 어두운 톤이 몸의 **33~42%**를 차지해 40px에서 검은 덩어리로 뭉쳤다(합격본 참나무는 26%이고 26/26/26으로 고르다). **가장 어두운 톤은 20%를 넘지 않는다** — 밝은 면과 중간 면이 몸의 대부분이고, 그늘은 아래 오른쪽 가장자리를 두르는 정도다.\n② **바탕보다 너무 어둡다.** 이 돌은 **눈밭·모래·마른 갯벌** 같은 밝은 바닥에 놓인다. 몸의 중간 톤은 회백~회갈(밝은 편)이어야 하고, 전체가 배경보다 훨씬 어두우면 돌이 아니라 **바닥에 뚫린 구멍**으로 보인다. 채도 낮은 초록빛 슬레이트 대신 **중성 회색~회갈**로.\n③ **넷의 팔레트와 분포가 똑같았다**(상위 5색 비율이 33/23/20/13/4로 소수점까지 일치). 실루엣만 다르고 색 구성이 같으면 여전히 복제로 읽힌다 — 넷의 밝기와 색조를 갈라라(밝은 회백 돌 / 회갈 돌 / 푸른기 도는 화강암 / 어둑한 현무암).\n④ **몸에서 삐져나온 가늘고 긴 돌기 금지.** 한 장에 오른쪽으로 뻗은 얇은 꼬리가 있어 화면에서 새·물고기로 보였다. 돌의 윤곽은 닫힌 덩어리다.", acnhRef: "바위", now: "procedural", pilot: 8, phase: 1 },
  ...
  { id: "snow-pile", nameKo: "눈 무더기", nameEn: "Snow pile", category: "ground", seasons: ["winter"], view: "stand", px: [40, 24], variants: 4, perScreen: 7, filler: true, brief: "바람에 쌓인 부드러운 눈 무더기, 위쪽은 희고 그늘은 옅은 청회색. 변형 2개.", now: "procedural", phase: 1 },
  ...
  { id: "sea-stack", nameKo: "시스택(갯바위 기둥)", nameEn: "Sea stack", category: "ground", seasons: ALL, view: "stand", px: [90, 130], variants: 3, perScreen: 3, brief: "파도가 깎다 남긴 바위 기둥 — 위로 갈수록 좁고 옆면에 가로 층리, 발치는 늘 젖어 어둡고 흰 따개비 띠가 한 줄. 변형 2개는 기울기가 다르다(이번 배치는 1개).", now: "none", pilot: 1, phase: 1 },
  ...
  { id: "cloud-low", nameKo: "구름(낮은)", nameEn: "Cloud, low", category: "sky", seasons: ALL, view: "flat", px: [190, 80], variants: 8, perScreen: 16, grid: 64, brief: "뭉게구름 — 위로 덩이가 부풀고 **아래쪽 윤곽이 대체로 수평**이다. 흰빛 #f6f8fb 3톤(윗면 밝고 밑면은 회청). 변형 4개는 덩이 수(2~5)와 너비가 확실히 다르게 — 같은 구름이 반복되면 하늘이 벽지가 된다. ⚠ **밑면을 별도의 판으로 그리지 말 것**: 1차 납품은 덩이 아래에 좌우로 더 튀어나온 평평한 선반이 붙어 탁자처럼 보였다. 아래 윤곽은 덩이들의 밑선이 이어져 수평에 가까워지는 것이지, 따로 붙인 널빤지가 아니다.", now: "procedural", phase: 1 },
  { id: "cloud-mid", nameKo: "구름(중간)", nameEn: "Cloud, mid", category: "sky", seasons: ALL, view: "flat", px: [230, 60], variants: 6, perScreen: 12, grid: 64, brief: "중간 높이의 구름 — 낮은 구름보다 옆으로 길고 납작하다(자리 230×60, 가로:세로 3.8:1), 덩이 경계가 부드럽지만 여전히 픽셀 계단. 변형 3개. ⚠ 뭉게구름과 같은 금지: **밑면에 따로 판을 붙이지 않는다.**", now: "procedural", phase: 1 },
  ...
  { id: "cloud-wisp", nameKo: "새털구름(짧은)", nameEn: "Cirrus, short", category: "sky", seasons: ALL, view: "flat", px: [140, 40], variants: 4, perScreen: 8, grid: 64, brief: "짧은 새털구름 — 긴 새털(cloud-high)과 **같은 어법·같은 두 톤 회백**이되 길이가 절반쯤인 조각(자리 140×40, 가로:세로 3.5:1). 획 두셋이 나란히 눕고 한쪽 끝이 갈고리로 살짝 굽는다. 긴 것 사이사이에 섞여 놓이므로 **긴 것을 그대로 잘라 놓은 것처럼 보이면 안 된다** — 끝이 뭉툭하게 맺히거나 살짝 뭉쳐 한 덩이로 읽히는 쪽. **변형 2개는 휜 방향이 서로 반대**(왼쪽 갈고리 / 오른쪽 갈고리)라 나란히 놓아도 복제로 안 보인다.", now: "procedural", pilot: 2, phase: 1 },
  { id: "cloud-storm", nameKo: "비구름", nameEn: "Rain cloud", category: "sky", seasons: ALL, view: "flat", px: [250, 110], variants: 4, perScreen: 8, grid: 64, brief: "비구름 — 아래쪽이 어둡고(회청) 위로 두껍게 솟는다. 자리 250×110(가로:세로 2.3:1)이니 옆으로 넓게. 변형 2개는 두께가 다르다. ⚠ 1차 납품의 두 가지 금지: **밑면의 평평한 판**(탁자 상판처럼 좌우로 튀어나왔다)과 **아래로 뻗은 사각 기둥 두 개**(탁자 다리처럼 보였다). 늘어진 자락이 필요하면 구름 몸통에서 **폭이 좁아지며 이어지는 덩이**로, 몸통과 떨어진 막대가 아니라.", now: "procedural", phase: 1 },
