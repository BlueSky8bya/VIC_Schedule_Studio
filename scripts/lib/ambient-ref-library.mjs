...
export function referenceDirectory({ workspaceRoot, category, entity = null, manifest }) {
  if (!Object.hasOwn(REFERENCE_CATEGORIES, category)) throw new Error(`Unknown reference category: ${category}`);
  if (entity === null) return referencePath(workspaceRoot, ["art-src", "reference", category]);
  ...
  return referencePath(workspaceRoot, ["art-src", category, entry.id, "레퍼런스"]);
}

/** Preserve both existing halves of a pair, including dangling links and manual sidecars. */
export function writeReferencePair({ workspaceRoot, category, entity = null, manifest, filename, image, card }) {
...
/** Only the common library and canonical entity reference directories are eligible. */
export function referenceDirectories({ workspaceRoot, manifest }) {
  ...
  return Object.keys(REFERENCE_CATEGORIES).flatMap((category) => [
    { category, dir: referenceDirectory({ workspaceRoot, category }) },
    ...entities.filter((entity) => entity.category === category).map((entity) => ({
      category, entity: entity.id, dir: referencePath(workspaceRoot, ["art-src", category, entity.id, "레퍼런스"]),
    })),
  ]);
}

export function parseReferenceFetchArgs(args, { workspaceRoot, manifest }) {
  const options = { limit: 24, query: "", entity: null, all: false }, categories = [];
  ...
  const targets = options.all ? Object.keys(REFERENCE_CATEGORIES) : [...new Set(categories)];
  if (!targets.length) throw new Error(`범주를 달라 — ${Object.keys(REFERENCE_CATEGORIES).join(" · ")} 또는 --all`);
  if (options.entity !== null && (options.all || targets.length !== 1)) throw new Error("--entity requires exactly one category and cannot use --all");
...
export function updateReferenceNotice({ workspaceRoot, manifest, check = false }) {
  ...
  const noticePath = referencePath(workspaceRoot, ["art-src", "reference", "NOTICE.md"]);
